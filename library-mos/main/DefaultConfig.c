...
int _DOpus_DefaultConfig(struct ConfigStuff *cstuff)
{
	...
	if((FindSystemFile("DirectoryOpus.DefCFG", name, 256, SYSFILE_DATA)) && (ReadConfig(name, cstuff)) == 1)
		return (1);
	...
	strcpy(config->output, "CON:20/10/600/180/Directory Opus Output/CLOSE");
	...
	for(i = 0; i < NUMFONTS; i++)
	{
		...
		strcpy(config->fontbufs[i], "topaz.font");
	}
...
//	config->screenmode = MODE_WORKBENCHCLONE;
	...
	/* Buttons */
	for(i = 0; i < 42; i++)
		AssignGadget(cstuff, 0, i, defgads[i], deffuncs[i]);
	for(i = 0; i < 42; i++)
		AssignGadget(cstuff, 0, i + 42, revgads[i], revfuncs[i]);
	for(i = 78; i < GADCOUNT; i++)
		AssignGadget(cstuff, 0, i, NULL, NULL);
	...
	for(i = 0; i < 5; i++)
	{
		if((type = (struct dopusfiletype *)LAllocRemember(&cstuff->typekey, sizeof(struct dopusfiletype), MEMF_CLEAR)))
		{

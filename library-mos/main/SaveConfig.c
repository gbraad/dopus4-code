...
int _DOpus_SaveConfig(char *name, struct ConfigStuff *cstuff)
{
	...
	while (bank)
	{
		if((Write(out, "\003G", 2)) < 2)
			goto error;
		if((Write(out, (char *)bank->gadgets, sizeof(struct dopusfunction) * GADCOUNT)) < sizeof(struct dopusfunction) * GADCOUNT)
			goto error;

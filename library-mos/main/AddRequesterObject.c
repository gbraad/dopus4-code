...
APTR _DOpus_AddRequesterObject(struct RequesterBase *reqbase, struct TagItem *taglist)
{
	...
	ULONG data;
	int tag, size = size, type = type, xoffset, yoffset;
	...
	for(tag = 0; tag > -1; tag++)
	{
		...
		case RO_Type:
			if(object)
				break;
			if(!(object = LAllocRemember(&reqbase->rb_memory, sizeof(struct RequesterObject), MEMF_CLEAR)))
				return (NULL);
			...
			if(!
			   (object->ro_data = LAllocRemember(&reqbase->rb_memory, size, MEMF_CLEAR)))
				return (NULL);
		...
		case RO_GadgetType:
			...
			case GADGET_STRING:
				type = GTYP_STRGADGET;
				if(!(stringinfo = (struct StringInfo *)LAllocRemember(&reqbase->rb_memory, sizeof(struct StringInfo), MEMF_CLEAR)))
					return (NULL);
			...
			case GADGET_PROP:
				...
				if(!(propinfo = (struct PropInfo *)LAllocRemember(&reqbase->rb_memory, sizeof(struct PropInfo), MEMF_CLEAR)))
					return (NULL);
				gadget->SpecialInfo = (APTR) propinfo;
				if(!(propimage = (struct Image *)LAllocRemember(&reqbase->rb_memory, sizeof(struct Image), MEMF_CLEAR)))
					return (NULL);
			...
			case GADGET_CHECK:
				...
				if(private)
				{
					if(!private->check_off_image)
						private->check_off_image = GetCheckImage(reqbase->rb_shadow, reqbase->rb_bg, 0, &reqbase->rb_memory);
					if(!private->check_on_image)
						private->check_on_image = GetCheckImage(reqbase->rb_shadow, reqbase->rb_bg, 1, &reqbase->rb_memory);
			...
			case GADGET_RADIO:
				...
				if(private)
				{
					if(!private->button_off_image)
						private->button_off_image = GetButtonImage(15, 9, reqbase->rb_shine, reqbase->rb_shadow, reqbase->rb_bg, reqbase->rb_bg, &reqbase->rb_memory);
					if(!private->button_on_image)
						private->button_on_image = GetButtonImage(15, 9, reqbase->rb_shine, reqbase->rb_shadow, reqbase->rb_fg, reqbase->rb_bg, &reqbase->rb_memory);
		...
		case RO_Text:
			...
			if(string && (*string = LAllocRemember(&reqbase->rb_memory, strlen((char *)data) + 1, 0)))
		...
		case RO_HighRecess:
			if(data && object)
			{
				...
				{
					AddGadgetBorders(&reqbase->rb_memory, gadget, 1, reqbase->rb_shine, reqbase->rb_shadow);
				}
	...
	{
		if(!(stringinfo->Buffer = LAllocRemember(&reqbase->rb_memory, stringinfo->MaxChars + 1, MEMF_CLEAR)))
			return (NULL);
	}

...
int _DOpus_DoSimpleRequest(struct Window *window, struct DOpusSimpleRequest *simple)
{
	...
	if(!(text = (char *)LAllocRemember(&key, textlen + 2, MEMF_CLEAR)) || !(keys = (char *)LAllocRemember(&key, gnum, MEMF_CLEAR)) || !(newwindow = (struct NewWindow *)LAllocRemember(&key, sizeof(struct NewWindow), MEMF_CLEAR)) || !(gadgets = (struct Gadget *)LAllocRemember(&key, sizeof(struct Gadget) * (num + 1), MEMF_CLEAR)) || !(contgad = (struct Gadget *)LAllocRemember(&key, sizeof(struct Gadget), MEMF_CLEAR)) || !(gadptr = (char **)LAllocRemember(&key, (num + hnum + 1) * 4, MEMF_CLEAR)) || !(buf = (char *)LAllocRemember(&key, sw + 2, 0)))
	{
		LFreeRemember(&key);
		return (0);
	}
	...
	if(simple->strbuf)
	{
		if(!(sinfo = (struct StringInfo *)LAllocRemember(&key, sizeof(struct StringInfo), MEMF_CLEAR)) || !(extend = (struct StringExtend *)LAllocRemember(&key, sizeof(struct StringExtend), MEMF_CLEAR)) || !(strbuf = (char *)LAllocRemember(&key, simple->strlen + 1, MEMF_CLEAR)))
		{
			LFreeRemember(&key);
			return (0);
		}
		...
		else if(simple->gads[b][0])
		{
			...
			else if(a < gnum)
			{
				...
				for(d = 0; simple->gads[b][d]; d++)
					...
					{
						keys[a] = ToLower(simple->gads[b][d + 1]);
						break;
					}
	...
	if((screen->Flags & 15) == CUSTOMSCREEN)
		ModifyIDCMP(Window, IDCMP_GADGETUP | IDCMP_VANILLAKEY | IDCMP_INACTIVEWINDOW | IDCMP_MOUSEBUTTONS);
	...
	for(a = 0; a < textlen; a++)
	{
		...
		if(text[a] == 10 || llen == sw)
		{
			if(simple->flags & SRF_NOCENTER)
				Move(rp, Window->BorderLeft + 6, ty);
			else
				Move(rp, (Window->Width - (TextLength(rp, buf, llen))) / 2, ty);
			Text(rp, buf, llen);
			llen = 0;
			ty += cy + 1;
			++dl;
			if(dl > lines && a < textlen - 1)
			{
				dl = 0;
				if(num)
					AddGadgets(Window, contgad, &(string_table[STR_CONTINUE])/*contstring */ , 1, simple->hi, simple->lo, 1);
				c = 0;
				FOREVER
				{
	...
	{
		SetAPen(rp, 0);
		RectFill(rp, gadgets[strgad].LeftEdge, gadgets[strgad].TopEdge, gadgets[strgad].LeftEdge + gadgets[strgad].Width - 1, gadgets[strgad].TopEdge + gadgets[strgad].Height - 1);
		Delay(5);

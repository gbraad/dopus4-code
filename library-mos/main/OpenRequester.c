...
struct Window *_DOpus_OpenRequester(struct RequesterBase *reqbase)
{
	...
	else if(reqbase->rb_screenname)
	{
		...
		if((screen = LockPubScreen(reqbase->rb_screenname)))
		{
	...
	for(attempt = 0; attempt < 2; attempt++)
	{
		...
		if(width <= screen->Width && height <= screen->Height)
		{
			...
			if((window = OpenWindow(&newwin)))
			{
				...
				if(!(reqbase->rb_flags & RBF_BORDERS))
				{
					Do3DBox(window->RPort, 2, 1, window->Width - 4, window->Height - 2, reqbase->rb_shine, reqbase->rb_shadow);
				}
				SetAPen(window->RPort, reqbase->rb_bg);
				RectFill(window->RPort, window->BorderLeft + 2, window->BorderTop + 1, window->Width - 3 - window->BorderRight, window->Height - 2 - window->BorderBottom);
		...
		if(attempt == 0)
		{
			if(!(reqbase->rb_font = OpenFont(&topazfont)))
				break;
	...
	reqbase->rb_privatedata = LAllocRemember(&reqbase->rb_memory, sizeof(struct PrivateData), MEMF_CLEAR);

...
static const struct PropInfo defaultprop =
{
...
static const struct Image defaultimage =
{
	0,
	0,
	8,
...
static const struct IntuiText defaulttext[6] =
{
...
static const struct Gadget defaultgadgets[9] =
{
...
static const struct NewWindow defaultwindow =
{
	15,
	25,
	316,
...
void freemulti(struct DOpusFileReq *freq)
{
	if(freq->flags & DFRF_MULTI)
		LFreeRemember(&freq->filearraykey);
	freq->filearray = NULL;
}
...
void close_req(struct FileReqData *freqdata)
{
	...
	if(freqdata)
	{
		...
		if(freqdata->flags & DFRF_MULTI && (file = freqdata->firstfile))
		{
			...
			{
				if(buf = (char *)LAllocRemember(&freq->filearraykey, b * FILEBUF_SIZE, MEMF_CLEAR))
				{
		...
		if(freqdata->finfo)
			FreeMem(freqdata->finfo, sizeof(struct FileInfoBlock));
...
void displayfiles(struct FileReqData *freqdata)
{
	...
	for(b = 0; b < freqdata->freq->lines; b++)
	{
		...
		if(work)
		{
			...
			Text(rp, work->name, a);
			if(a < nlim)
				Text(rp, nullstring, nlim - a);
			if(slim)
				Text(rp, work->size, slim);
			if(work->select)
				SetBPen(rp, 0);
			work = work->next;
		}
		else
			Text(rp, nullstring, freqdata->width);
		y += freqdata->fh;
	}
}
...
void addfileentry(struct FileReqData *freqdata, char *name, int type, int size)
{
	...
	if(freqdata->firstfile)
	{
		...
		while (add)
		{
			if(LStrCmpI(name, add->name) <= 0 || (type > 0 && add == freqdata->firstdir))
			{
				if(add->last)
					afterdir = add->last;
				else
	...
	if(!(new = (struct direntry *)LAllocRemember(&freqdata->filekey, sizeof(struct direntry), MEMF_CLEAR)))
		return;
	...
	if(type == 0)
		strcpy(work->size, devtypes[size]);
	else if(type < 0)
	{
		...
		strcpy(work->size, string_table[STR_FILEREQ_DIR]);
	work->type = type;
...
int get_filenames(struct FileReqData *freqdata)
{
	...
	while (ExNext(lock, freqdata->finfo))
	{
		...
		if(freqdata->finfo->fib_DirEntryType < 0)
		{
			...
			{
				int32 i;
				StrToLong(freqdata->finfo->fib_FileName, &i);
				LSprintf(buf, "%ld", i);
				if(LStrCmp(buf, freqdata->finfo->fib_FileName))
					continue;
			}
...
int getnew_file(struct FileReqData *freqdata, struct direntry *work)
{
	...
	if((LStrCmp(freqdata->freq->filebuf, work->name)) == 0)
		b = 1;
	else
		b = 0;
...
{
	DebugPrintF("listdevices() Needs Fixing ASAP\n");
...
*//*	char devname[32];
	...
	while(devlist)
	{
		...
		{
			CopyStringBSTRToC(devlist->dl_Name, devname, 32);
			LStrCat(devname, ":");
...
/* this code works, needs adapting only:
	...
	while((dl = NextDosEntry(dl, LDF_DEVICES)))
	{
		...
		{
			CopyStringBSTRToC(dl->dol_Name, pathname, 256);
		...
		if(code == SELECTDOWN && x > 9 && x < freqdata->ww + 10 && y >= freqdata->fy && y < freqdata->fy + (freqdata->freq->lines * freqdata->fh))
		{
			...
			if(file = getfileentry(freqdata, a))
			{
				...
				if(b)
				{
					...
					if((ret = getnew_file(freqdata, file)) == 1 && DoubleClick(freqdata->prevsec, freqdata->prevmic, sec, mic))
						ret = OKAY;
...
int initrequester(struct FileReqData *freqdata)
{
	...
	freqdata->version2 = 1;
	screen = (freq->window) ? freq->window->WScreen : ((struct IntuitionBase *)(Data.LibBase))->ActiveScreen;
	...
	if(screen->RastPort.Font->tf_Flags & FPF_PROPORTIONAL)
		font = ((struct GfxBase *)(Data.LibBase))->DefaultFont;
	else
	...
	if(!(freqdata->fr_Window = OpenWindow(&freqdata->reqwin)))
		return (0);
	rp = freqdata->fr_Window->RPort;
	SetFont(rp, font);
	SetDrMd(rp, JAM2);
	...
	for(b = 2; b < 8; b++)
		Do3DBox(rp, freqdata->reqgads[b].LeftEdge + 2, freqdata->reqgads[b].TopEdge + 1, freqdata->reqgads[b].Width - 4, freqdata->reqgads[b].Height - 2, 2, 1);
	if(!(freqdata->flags & DFRF_DIRREQ))
	...
	for(b = 4; b < 6; b++)
		DoArrow(rp, freqdata->ww + 14, freqdata->reqgads[b].TopEdge + 1, 12, 6, 1, 0, b - 4);

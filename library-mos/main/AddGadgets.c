...
int _DOpus_AddGadgets(struct Window *win, struct Gadget *firstgad, char **text, int count, int fg, int bg, int add)
{
	...
	while (gad && num < count)
	{
		...
		{
			Do3DStringBox(rp, gad->LeftEdge - (gad->MutualExclude * 2), gad->TopEdge - gad->MutualExclude, gad->Width + (gad->MutualExclude * 4), gad->Height + (gad->MutualExclude * 2), fg, bg);
		}
		else if(gad->GadgetType == GTYP_PROPGADGET)
		{
			Do3DBox(rp, gad->LeftEdge - 2, gad->TopEdge - 1, gad->Width + 4, gad->Height + 2, fg, bg);
		}
		else if(gad->MutualExclude == GAD_CYCLE)
		{
			Do3DCycleBox(rp, gad->LeftEdge + 2, gad->TopEdge + 1, gad->Width - 4, gad->Height - 2, fg, bg);
		}
		else if(gad->MutualExclude == GAD_CHECK)
		{
			Do3DBox(rp, gad->LeftEdge + 2, gad->TopEdge + 1, 22, 9, fg, bg);
		}

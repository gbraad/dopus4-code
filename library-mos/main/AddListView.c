...
int _DOpus_AddListView(struct DOpusListView *view, int count)
{
	struct RastPort *rp;
	int a, addcount = 0;
	...
	while (view && (addcount < count))
	{
		...
		if(!(view->listgads = (struct Gadget *)LAllocRemember(&view->key, sizeof(struct Gadget) * 3, MEMF_CLEAR)) || !(view->listimage = (struct Image *)LAllocRemember(&view->key, sizeof(struct Image), MEMF_CLEAR)) || !(view->listprop = (struct PropInfo *)LAllocRemember(&view->key, sizeof(struct PropInfo), MEMF_CLEAR)) || !(view->selected = (char *)LAllocRemember(&view->key, view->count, MEMF_CLEAR)))
		{
		...
		if(view->flags & DLVF_DUMB)
			...
			Do3DBox(rp, view->x, view->y, view->w, view->h, view->boxhi, view->boxlo);
		...
		if(view->title)
		{
			...
			if(view->flags & DLVF_TTOP)
				Move(rp, view->x, (view->y - view->fh - 2) + view->fb);
			else
				Move(rp, view->x - 6 - (a * view->fw), view->yo + view->fb);

...
int _DOpus_FSSetMenuStrip(struct Window *window, struct Menu *firstmenu)
{
	struct Menu *menu;
	...
	if((drinfo = GetScreenDrawInfo(window->WScreen)))
	{
	...
	while(menu)
	{
		if(xpos == 0)
			...
			menu->LeftEdge = xpos;
		menu->Width = TextLength(&window->WScreen->RastPort, menu->MenuName, strlen(menu->MenuName)) + window->WScreen->RastPort. Font->tf_XSize;
		xpos += menu->Width + 16;
		...
		while(item)
		{
  			...
  			if(item->Flags & ITEMTEXT)
			{
				...
				while(text)
				{
					...
					if(first == 0)
					{
						if(LStrnCmp((char *)text->IText, "   ", 3) == 0)
				...
				if(item->Flags & COMMSEQ)
					mwidth += comwidth + TextLength(&window->WScreen->RastPort, (char *)&item->Command, 1);
				if(mwidth > iwidth)
					iwidth = mwidth;

...
dodriveconfig()
{
	...
	gadkey = NULL;
	if(!(drivegads = (struct Gadget *)IDOpus->LAllocRemember(&gadkey, sizeof(struct Gadget) * USEDRIVECOUNT, MEMF_CLEAR)))
		return (0);
		...
		while(IMsg = getintuimsg())
		{
				...
				case 0:
					...
					case 1:
						if(doload(CFG_DRIVE, a))
						{
							showdrives();
							if(a)
								dogadgetinfo(cfg_string[STR_NEW_DRIVE_BANKS_LOADED]);
							else
								dogadgetinfo(cfg_string[STR_DEFAULT_DRIVE_BANKS_RESET]);
						}
						break;
					}
					break;
			...
			case IDCMP_RAWKEY:
				if(!seldrive || !editon || (code >= 0x60 && code <= 0x67) || code & 0x80)
					break;
				if(editdrivegadgets[1].Flags & GFLG_SELECTED)
				{
			...
			case IDCMP_MOUSEBUTTONS:
				...
				else if(code == MENUDOWN)
				{
					...
					if(fixrmbimage(&sampleclearrmb, NULL, 0, x, y))
					{
						...
						fixrmbimage(&sampleclearrmb, NULL, 1, -1, -1);
						if(a == 0)
							showkey(seldrive->key = 0xFF, seldrive->qual = 0, x_off + 150, y_off + 129, 331, 10);
						editdrivegadgets[1].Flags &= ~GFLG_SELECTED;
						help_ok = 1;
					}
				}
			...
			case IDCMP_GADGETUP:
					...
					if(mode > -1)
					{
						...
						else
						{
							...
							else
							{
								...
								{
								case DRIVE_COPYDRIVE:
									IExec->CopyMem((char *)seldrive, (char *)seconddrive, sizeof(struct dopusfunction));
								...
								case DRIVE_COPYBANK:
									if(dnum != odnum)
									{
										for(a = 0; a < 6; a++)
										{
											IExec->CopyMem((char *)&config->drive[(odnum * 6) + a], (char *)&config->drive[(dnum * 6) + a], sizeof(struct dopusfunction));
								...
								case DRIVE_SWAPBANK:
									if(dnum != odnum)
										SwapMem((char *)&config->drive[odnum * 6], (char *)&config->drive[dnum * 6], sizeof(struct dopusfunction) * 6);
									break;
								}
								seldrive = NULL;
								showdrives();
					...
					else
					{
						...
						else
						{
							...
							if(!editon)
							{
								...
								editdrivegadgets[1].Flags &= ~GFLG_SELECTED;
								IDOpus->AddGadgets(Window, editdrivegadgets, drivegadgets2, 4, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
								fixrmbimage(&sampleclearrmb, &editdrivegadgets[1], 2, -1, -1);
								doglassimage(&editdrivegadgets[2]);
							}
							else
							{
								IDOpus->RefreshStrGad(&editdrivegadgets[0], Window);
								IDOpus->RefreshStrGad(&editdrivegadgets[3], Window);
							}
							if(editdrivegadgets[1].Flags & GFLG_SELECTED)
							{
								fixrmbimage(&sampleclearrmb, NULL, 0, -1, -1);
				...
				case DRIVE_EDITSAMPLE:
					help_ok = (editdrivegadgets[1].Flags & GFLG_SELECTED) ? 0 : 1;
					break;
				...
				case DRIVE_OKAY:
					doundo(undo, 0);
					IIntuition->RemoveGList(Window, drivegads, USEDRIVECOUNT);
...
void showdrivename(struct dopusfunction *drive, struct Gadget *gadget)
{
	int a, x, y, l;
	char showname[16];
	...
	y = gadget->TopEdge;
	if(drive)
		IGraphics->SetAPen(rp, screen_pens[drive->bpen].pen);
	else
...
int docolourgad(struct RastPort *r, int x, int y, int mx, int my, STRPTR name, int *fp, int *bp, int type)
{
	...
	if(my > y && my < y + 21)
	{
		for(box = 0; box < 2; box++)
		{
			...
			{
				if(mx >= colour_box_xy[box][num][0] && mx <= colour_box_xy[box][num][2] && my >= colour_box_xy[box][num][1] && my <= colour_box_xy[box][num][3])
				{
					if((box == 0 && *fp != num) || (box == 1 && *bp != num))
					{
						...
						if(box == 0)
							...
							*bp = num;
						if(name)
							showfuncob(r, name, *fp, *bp, type, x + 94, y - 10);
						return (1);
					}
					return (0);
				}
...
void driveeditoff()
{
	...
	seligad = NULL;
	IGraphics->SetAPen(rp, screen_pens[0].pen);
	IGraphics->RectFill(rp, x_off + 2, y_off + 65, x_bot, y_off + 140);
	IIntuition->RemoveGList(Window, editdrivegadgets, 4);
}

void deletedrive(struct dopusfunction *drive)
{
	drive->name[0] = 0;
	freestring(drive->function);
	drive->function = NULL;
	drive->key = 0xff;
	drive->qual = 0;
	if(paint_state)

				...
				case IDCMP_MENUPICK:
					...
					case 0:
						...
						case 1:
							if(doload(CFG_FILETYPE, a))
							{
								...
								showtypelist(listptr);
								if(a)
									dogadgetinfo(cfg_string[STR_NEW_FILETYPES_MERGED]);
								else
									dogadgetinfo(cfg_string[STR_DEFAULT_FILETYPES_MERGED]);
							}
							break;
						}
						break;
					case 1:
						...
						case 0:
							...
							break;
						case 1:
							if(!(fclass = getfileclasslist(1)))
								break;
							if(editclass(fclass, 0))
								classchange = 1;
							break;
						case 2:
							...
							break;
						case 3:
							if(!(fclass = getfileclasslist(2)))
								break;
							sprintf(buf, "\"%s\"\n%s", fclass->type, cfg_string[STR_REALLY_DELETE_THIS_CLASS]);
							if(request(buf))
							{
				...
				case IDCMP_GADGETUP:
					...
					case FILETYPE_CANCEL:
						if(classchange && !(request(cfg_string[STR_EXIT_WITHOUT_SAVING_CLASSES])))
							break;
						if(gadgetid == FILETYPE_CANCEL)
							doundo(undo, UNDO_FILETYPE);
						doundo(undo, 0);
						IDOpus->RemoveListView(&filetypeactionlist, 1);
						IDOpus->LFreeRemember(&key);
						freefileclasses();
						IIntuition->ClearMenuStrip(Window);
						projectmenu.NextMenu = NULL;
						IDOpus->FSSetMenuStrip(Window, &projectmenu);
						return ((gadgetid == FILETYPE_OKAY));
					case FILETYPE_NEW:
						dogadgetinfo(NULL);
						if(!(fclass = getfileclasslist(0)))
							break;
						type = firsttype;
						while(type)
						{
							if(strcmp(type->type, fclass->type) == 0)
								break;
							type = type->next;
						}
...
char **makefiletypelist(struct DOpusRemember **key)
{
	char **list;
	int count, a;
	...
	if(!(list = IDOpus->LAllocRemember(key, (count + 1) * 4, MEMF_CLEAR)))
		return (NULL);
	type = firsttype;
	for(a = 0; a < count; a++)
	{
...
void showtypelist(STRPTR *list)
{
	filetypeactionlist.items = list;
...
void filetypetitle(struct dopusfiletype *type)
{
	char title[80];
...
int editfiletype(struct dopusfiletype *type, struct DOpusRemember **key, int new)
{
	...
	else
	{
		...
		{
			if(b)
				func[a] = getcopy(type->function[a], -1, &typekey);
			else
				func[a] = oldfunc[a];
			freestring(type->function[a]);
			type->function[a] = func[a];
...
int editclass(struct fileclass *class, int new)
{
	...
	{
		c = 1;
		IDOpus->RemoveListView(&filetypeactionlist, 2);
	}
	cleanconfigscreen();
...
void addfiletype(struct dopusfiletype *type)
{
...
void readfileclasses()
{
	...
	if(in = IDOS->Open(classname, MODE_OLDFILE))
	{
		...
		for(;;)
		{
			...
			for(lsize = pos; lsize < size; lsize++)
				if(!classbuf[lsize])
					break;
			if(!(addfileclass(buf, typeid, &classbuf[pos])))
				break;
			pos = lsize + 1;
		}
	}
...
int importfileclasses()
{
	...
	{
		sprintf(buf2, cfg_string[STR_OPEN_FAILED], IDOS->IoErr());
		if(!(request(buf2)))
			return (0);
	}
	...
	if((IDOpus->CheckExist(buf, &size)) >= 0 || !(classbuf = IDOpus->LAllocRemember(&key, size + 2, MEMF_CLEAR)))
	{
		IDOS->Close(in);
		return (0);
	}
	...
	for(a = 0, num = 0; a < size; a++)
		if(!classbuf[a])
			++num;
	num /= 2;

	if((classlist = IDOpus->LAllocRemember(&key, (num + 1) * 4, MEMF_CLEAR)) && (classtypeid = IDOpus->LAllocRemember(&key, (num + 1) * 4, MEMF_CLEAR)) && (classrecog = IDOpus->LAllocRemember(&key, (num + 1) * 4, MEMF_CLEAR)) && (classarray = IDOpus->LAllocRemember(&key, num + 1, MEMF_CLEAR)))
	{
		pos = 0;
		a = 0;
		for(;;)
		...
		if(classlist[0] && (dolistwindow(cfg_string[STR_SELECT_CLASSES_TO_IMPORT], 332, 72, classlist, DLVF_MULTI | DLVF_HIREC, classarray, NULL)))
		{
			for(a = 0; a < num; a++)
			{
				if(classarray[a])
				{
					if(!(addfileclass(classlist[a], classtypeid[a], classrecog[a])))
						break;
					ret = 1;
				}
			}
...
int savefileclasses()
{
	...
	while(fclass)
	{
		if(fclass->type[0])
		{
			if((IDOS->Write(out, fclass->type, (a = strlen(fclass->type)))) < a)
				break;
			if(fclass->typeid[0])
			{
				IDOS->Write(out, &g, 1);
				if((IDOS->Write(out, fclass->typeid, (a = strlen(fclass->typeid)))) < a)
					break;
			}
		}
		IDOS->Write(out, &f, 1);
...
void removefileclass(struct fileclass *fclass)
{
	if(fclass->last)
		fclass->last->next = fclass->next;
	if(fclass->next)
...
int editfileclass(struct fileclass *fclass, int new)
{
	...
	char *classlist[MAXFUNCS + 1], classtype[MAXFUNCS], *displist[MAXFUNCS + 1], *templist, temptype, buf[16];
	unsigned char c;

	for(a = 0; a < MAXFUNCS; a++)
	{
		displist[a] = IDOpus->LAllocRemember(&key, 80, MEMF_CLEAR);
		classlist[a] = NULL;
	}
	...
	IDOpus->AddGadgetBorders(&key, &editclassgadgets[2], 4, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	IDOpus->AddGadgetBorders(&key, &editclassgadgets[6], 3, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	IDOpus->AddGadgetBorders(&key, &editclassgadgets[10], 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	IDOpus->AddGadgetBorders(&key, &editclassgadgets[12], 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);

	IGraphics->SetAPen(rp, screen_pens[1].pen);
	IDOpus->AddGadgets(Window, editclassgadgets, editclassgads, 14, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	doglassimage(&editclassgadgets[6]);
	doglassimage(&editclassgadgets[8]);
	IDOpus->Do3DBox(rp, x_off + 37, y_off + 140, 116, 12, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);

	fileview_buf = NULL;
	fileview_lines = fileview_topline = 0;
	fileview_type = 1;
		...
		while(IMsg = getintuimsg())
		{
			if((view = IDOpus->ListViewIDCMP(&editclasslist, IMsg)) == (struct DOpusListView *)-1)
			{
				...
				case IDCMP_MOUSEBUTTONS:
					if(code != SELECTDOWN || mx < x_off + 11 || mx > x_off + 505 || my < y_off + 21 || my > x_off + 68)
						break;
					if(oldsec && IIntuition->DoubleClick(oldsec, oldmic, sec, mic))
						off = 1;
					else
						off = 0;
					oldsec = sec;
					...
					for(;;)
					{
						if(my > y_off + 20 && my < y_off + 69)
						{
							...
							if(off)
							{
								...
								{
									old = fileview_offset;
									if((fileview_offset = a) < 0 || fileview_offset >= fileview_size)
										fileview_offset = -1;
									a = 0;
									b = 1;
								}
							...
							else
							{
								...
								if((fileview_position = (y * 16) + x) < 0 || fileview_position >= fileview_size)
									fileview_position = -1;
								a = fileview_position;
								b = 0;
								if(fileview_offset > fileview_position)
						...
						if(my < y_off + 21)
						{
							...
							{
								--fileview_topline;
								fileview_oldtop = fileview_position = -1;
								show_file_view();
								IDOpus->FixSliderPot(Window, &editclassgadgets[11], fileview_topline, fileview_lines, 6, 1);
							}
						}
						else if(my > y_off + 68)
						{
				...
				case IDCMP_RAWKEY:
					...
					{
					case CURSORUP:
						if(fileview_position > 15)
							cursor_fileview(-16);
						break;
					case CURSORDOWN:
						if(fileview_position < fileview_size - 16)
							cursor_fileview(16);
						break;
					case CURSORLEFT:
						if(fileview_position)
							cursor_fileview(-1);
						break;
					case CURSORRIGHT:
						if(fileview_position < fileview_size - 1)
							cursor_fileview(1);
						break;
					default:
						{
							...
							if(c == getkeyshortcut(cfg_string[STR_EDITCLASS_FILECLASS]))
								...
								IDOpus->ActivateStrGad(&editclassgadgets[1], Window);
				...
				case IDCMP_GADGETDOWN:
					...
					case CLASS_VIEWSLIDER:
						...
						for(;;)
						{
							...
							{
								fileview_topline = IDOpus->GetSliderPos(&editclassgadgets[11], fileview_lines, 6);
							}
							if(fileview_topline != fileview_oldtop)
								show_file_view();
						...
						IDOpus->ShowSlider(Window, &editclassgadgets[11]);
						break;
					}
					break;
				...
				case IDCMP_GADGETUP:
					...
					case CLASS_CANCEL:
						if(editclassgadgets[3].Flags & GFLG_DISABLED)
							seteditclassgads(1);
						IDOpus->RemoveListView(&editclasslist, 1);
						IDOpus->LFreeRemember(&key);
					...
					case CLASS_NEWENTRY:
						...
						{
							if(editclasslist.count >= MAXFUNCS)
								break;
							if(a > -1)
								selitem = a;
							else
					...
					case CLASS_HEXDEC:
						...
						IDOpus->DoCycleGadget(&editclassgadgets[10], Window, fileview_types, fileview_type);
					...
					case CLASS_TYPE:
						...
						break;
					case CLASS_TYPEID:
						IDOpus->StrToUpper(edit_typeidbuf, edit_typeidbuf);
						IDOpus->RefreshStrGad(&editclassgadgets[1], Window);
						break;
					}
					break;
				}
			...
			else if(view)
			{
				do
				{
					...
					{
						endclassedit(selitem, classlist, classtype, displist);
						if(editclassgadgets[4].Flags & GFLG_SELECTED)
						{
							checkclassswap();
							seteditclassgads(0);
...
void makeeditclasslist(struct fileclass *class, STRPTR *classlist, STRPTR classtype)
{
	...
	for(a = 0; a <= len; a++)
	{
		...
		else if(pos == 255 || class->recognition[a] > FTYC_ENDLIMIT || !class->recognition[a])
		{
			...
			buf[pos++] = class->recognition[a];
	}
	for(a = num; a < MAXFUNCS; a++)
		classlist[a] = NULL;
}

void dispclasslist(STRPTR *classlist, STRPTR classtype, STRPTR *displist)
{
	int a;
	char buf[80], *ptr;
...
void seteditclassgads(int on)
{
	...
	{
		for(a = 3; a < 6; a++)
			IDOpus->EnableGadget(&editclassgadgets[a], rp, 4, 2);
		IDOpus->EnableGadget(&editclassgadgets[6], rp, 4, 2);
		IDOpus->EnableGadget(&editclassgadgets[7], rp, 0, 0);
	}
	else
	{
		for(a = 3; a < 6; a++)
			IDOpus->DisableGadget(&editclassgadgets[a], rp, 4, 2);
		IDOpus->DisableGadget(&editclassgadgets[6], rp, 4, 2);
		IDOpus->DisableGadget(&editclassgadgets[7], rp, 0, 0);
	}
}

void removeclassentry(STRPTR *classlist, STRPTR classtype, int entry)
{
	if(entry < MAXFUNCS - 1)
	{
		IExec->CopyMem(&classlist[entry + 1], &classlist[entry], (MAXFUNCS - 1 - entry) * 4);
		IExec->CopyMem(&classtype[entry + 1], &classtype[entry], (MAXFUNCS - 1 - entry));
	}
	classlist[MAXFUNCS - 1] = NULL;
}

void insertnewclass(STRPTR *classlist, STRPTR classtype, int entry, STRPTR *displist, STRPTR string, int type)
{
	char *templist[MAXFUNCS], temptype[MAXFUNCS];
	int a;

	a = MAXFUNCS - entry - 1;
	IExec->CopyMem(&classlist[entry], templist, a * 4);
	IExec->CopyMem(templist, &classlist[entry + 1], a * 4);
	classlist[entry] = getcopy(string, -1, NULL);
	IExec->CopyMem(&classtype[entry], temptype, a);
	IExec->CopyMem(temptype, &classtype[entry + 1], a);
	classtype[entry] = type;
	dispclasslist(classlist, classtype, displist);
	if(entry < editclasslist.topitem || entry >= editclasslist.topitem + editclasslist.lines)
		editclasslist.topitem = entry;
...
void endclassedit(int item, STRPTR *classlist, STRPTR classtype, STRPTR *displist)
{
	freestring(classlist[item]);
	if(classtype[item] < 1 || classopvals[classtype[item] - 1] < FTYC_COMMANDOK)
	{
		if(edit_funcbuf[0])
			classlist[item] = getcopy(edit_funcbuf, -1, NULL);
		else
...
void showclassop(int op)
{
	...
	IGraphics->Move(rp, x, y_off + 148);
	IGraphics->Text(rp, ptr, a);
}

struct fileclass *getfileclasslist(int type)
{
	struct DOpusRemember *key = NULL;
	int a;
...
void draw_file_view()
{
	...
	IGraphics->SetAPen(rp, screen_pens[0].pen);
	IGraphics->RectFill(rp, y_off + 7, y_off + 21, x_off + 511, y_off + 68);
	do3dbox(x_off + 7, y_off + 21, 505, 48);
	IDOpus->Do3DBox(rp, x_off + 534, y_off + 31, 91, 12, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	IDOpus->Do3DBox(rp, x_off + 534, y_off + 57, 91, 12, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	IGraphics->SetAPen(rp, screen_pens[1].pen);
	IDOpus->UScoreText(rp, cfg_string[STR_POSITION], x_off + 535, y_off + 27, -1);
	IDOpus->UScoreText(rp, cfg_string[STR_OFFSET], x_off + 535, y_off + 53, -1);
	for(a = 0; a < 2; a++)
		IDOpus->DoArrow(rp, editclassgadgets[12 + a].LeftEdge + 2, editclassgadgets[12 + a].TopEdge + 1, 12, 6, screen_pens[1].pen, screen_pens[0].pen, a);
	fix_slider(&editclassgadgets[11]);
	IDOpus->FixSliderBody(Window, &editclassgadgets[11], fileview_lines, 6, 0);
	IDOpus->FixSliderPot(Window, &editclassgadgets[11], fileview_topline, fileview_lines, 6, 0);
	IDOpus->ShowSlider(Window, &editclassgadgets[11]);
	IDOpus->DoCycleGadget(&editclassgadgets[10], Window, fileview_types, fileview_type);
}
...
void load_file_view()
{
	...
	free_file_view();
	busy();
	if((IDOpus->CheckExist(edit_pathbuf, &fileview_size)) < 0 && (file = IDOS->Open(edit_pathbuf, MODE_OLDFILE)))
	{
		if(fileview_size > 4096)
			fileview_size = 4096;
...
void file_view_text(STRPTR txt, int line)
{
	int len;

	IGraphics->SetAPen(rp, screen_pens[1].pen);
	IGraphics->SetBPen(rp, screen_pens[0].pen);
	IGraphics->Move(rp, x_off + 9, y_off + 27 + (line * 8));
	if(txt)
		IGraphics->Text(rp, txt, (len = strlen(txt)));
	else
		len = 0;
	if(len < 62)
		IGraphics->Text(rp, spacestring, 62 - len);
}
...
void show_file_view(void)
{
	...
	for(line = 0; line < 6; line++)
	{
		if(line >= top && line <= bottom)
		{
			if(scroll)
			{
				IGraphics->ScrollRaster(rp, 0, scroll, x_off + 7, y_off + 21, x_off + 511, y_off + 68);
				scroll = 0;
			}
			if(fileview_lines <= line + fileview_topline)
				file_view_text(NULL, line);
			else
			{
				...
				if(px > -1 && px != ox)
				{
					...
					IGraphics->RectFill(rp, x_off + 9 + (10 + px) * char_w, y_off + 21 + (line * 8), x_off + 8 + (12 + px) * char_w, y_off + 28 + (line * 8));
					IGraphics->RectFill(rp, x_off + 9 + (10 + apx) * char_w, y_off + 21 + (line * 8), x_off + 8 + (11 + apx) * char_w, y_off + 28 + (line * 8));
					IGraphics->SetDrMd(rp, JAM2);
				}
				if(ox > -1)
				{
					...
					IGraphics->Move(rp, x_off + 9 + (10 + aox) * char_w, y_off + 27 + (line * 8));
					IGraphics->Text(rp, buf2, 1);
					IGraphics->SetAPen(rp, screen_pens[1].pen);
				}
			}
		}
...
void show_view_number(int num, int pos)
{
	char buf[20];
	int y;
	...
	{
		IGraphics->SetAPen(rp, screen_pens[0].pen);
		IGraphics->RectFill(rp, x_off + 534, y_off + 31 + (26 * pos), x_off + 624, y_off + 42 + (26 * pos));
		IGraphics->SetAPen(rp, screen_pens[1].pen);
	}
	else
	{
...
void cursor_fileview(int delta)
{
	int ups = 0;

	fileview_position += delta;
	if(fileview_position < 0)
		fileview_position = 0;
	else if(fileview_position >= fileview_size)
		fileview_position = fileview_size - 1;

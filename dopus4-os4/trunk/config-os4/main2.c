...
int dogadgetconfig()
{
	...
	if(!(gadgads = (struct Gadget *)IDOpus->LAllocRemember(&gadkey, sizeof(struct Gadget) * GADCOUNT, MEMF_CLEAR)))
		return (0);

	IDOpus->CreateGadgetBorders(&gadkey, 80, 10, &selborder, &unselborder, 0, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	...
	for(a = 0; a < 5; a++)
	{
		if(config->gadgetrows == rowtrans[a])
			gadrowsitems[a].Flags |= CHECKED;
		else
			gadrowsitems[a].Flags &= ~CHECKED;
	}
		...
		while(IMsg = getintuimsg())
		{
					...
					case 1:
						if(doload(CFG_GADGET, a))
						{
							if(bank = firstbank)
							{
								...
								showgadgets(currentbank, 1);
								if(a)
									dogadgetinfo(cfg_string[STR_NEW_GADGET_BANKS_APPENDED]);
								else
									dogadgetinfo(cfg_string[STR_DEFAULT_GADGET_BANKS_APPENDED]);
							}
						}
						break;
					}
			...
			case IDCMP_MOUSEBUTTONS:
				if(code == MENUDOWN)
				{
					...
					if((a = IDOpus->DoRMBGadget(&nextbankrmb, Window)) == 0)
					{
						if(currentbank)
						{
							if(mode == -1)
								dogadgetinfo(NULL);
							if(selgad && selbank == currentbank)
								select_gadget(seligad, 0);
							bank2 = currentbank;
							if(!(bank = lastbank(currentbank)))
							{
								bank = firstbank;
								while(bank && bank->next)
									bank = bank->next;
							}
							currentbank = bank;
			...
			case IDCMP_GADGETUP:
						...
						else
						{
							...
							else
							{
								if(mode == GAD_COPYGADGET)
								{
									...
									IExec->CopyMem((char *)selgad, (char *)secondgad, sizeof(struct newdopusfunction));
				...
				case GAD_NEXTBANK:
					bank2 = currentbank;
					if(!(currentbank = currentbank->next))
						currentbank = firstbank;
					if(currentbank != bank2)
						showgadgets(currentbank, 1);
					if(mode == -1)
				...
				case GAD_COPYBANK:
					...
					else if(!makenewbank(0))
						break;
					IExec->CopyMem((char *)bank2, (char *)currentbank, sizeof(struct newdopusfunction) * GADCOUNT);
				...
				case GAD_DELETEBANK:
					...
					{
						if(!request(cfg_string[STR_ONLY_BANK_CONFIGURED]))
							break;
					}
				...
				case GAD_OKAY:
					doundo(undo, 0);
					IIntuition->RemoveGList(Window, gadgads, GADCOUNT);
					IDOpus->LFreeRemember(&gadkey);
					gadgads = NULL;
					IIntuition->ClearMenuStrip(Window);
...
void showgadgets(struct dopusgadgetbanks *bank, int show)
{
	int a;
	...
	if(show)
	{
		for(a = 0; a < GADCOUNT; a++)
		{
			if(bank)
			{
				...
				if(selgad == &bank->gadgets[a])
					select_gadget(&gadgads[a], 1);
			}
			else
				showgadgetname(NULL, &gadgads[a]);
		}
	}
	if(bank)
	{
		...
		ptr = cfg_string[STR_GADGET_SCREEN_NO_BANKS];
	doscreentitle(ptr);
}

void showgadgetname(struct newdopusfunction *gad, struct Gadget *gadget)
{
	int a, x, y, l;
	...
	if(gad)
		IGraphics->SetAPen(rp, screen_pens[gad->bpen].pen);
	else
		IGraphics->SetAPen(rp, screen_pens[0].pen);
	IGraphics->RectFill(rp, x + 2, y + 1, x + 77, y + 8);
	if(gad)
	{
		IGraphics->SetAPen(rp, screen_pens[gad->fpen].pen);
		IGraphics->SetBPen(rp, screen_pens[gad->bpen].pen);
		if(gad->name && gad->name[0])
		{
			IGraphics->Move(rp, x + ((80 - (((a = strlen(gad->name)) > l ? (a = l) : a) * rp->Font->tf_XSize)) / 2), y + 7);
			IGraphics->Text(rp, gad->name, a);
		}
	}
	IGraphics->SetBPen(rp, screen_pens[0].pen);
}

int makenewbank(int insert)
{
	struct dopusgadgetbanks *bank, *bank2;
	int a;

	if(!(bank = IExec->AllocMem(sizeof(struct dopusgadgetbanks), MEMF_CLEAR)))
	{
...
struct dopusgadgetbanks *lastbank(struct dopusgadgetbanks *bank)
{
	struct dopusgadgetbanks *temp;

	temp = firstbank;
	while(temp && temp->next != bank)
		temp = temp->next;
	return (temp);
}

void dogadgetinfo(STRPTR str)
{
	int tx, ex, len, x;

	x = (Window->Width - 580) / 2;

	IDOpus->Do3DBox(rp, x, y_off + 144, 580, 10, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	...
	if(str)
	{
		...
		IDOpus->UScoreText(rp, str, tx, y_off + 151, -1);
		ex = rp->cp_x;
	}
	else
		ex = x;
	IGraphics->SetAPen(rp, screen_pens[0].pen);
	if(tx > x)
		IGraphics->RectFill(rp, x, y_off + 145, tx - 1, y_off + 153);
	if(ex < x + 579)
		IGraphics->RectFill(rp, ex, y_off + 145, x + 579, y_off + 153);
}
...
int domenuconfig()
{
	...
	if(!(menugads = (struct Gadget *)IDOpus->LAllocRemember(&menukey, sizeof(struct Gadget) * (MENUCOUNT + 5 - (HIDDENLINES * 5)), MEMF_CLEAR)))
		return (0);
	selgad = NULL;
	selbank = NULL;
	menuoffset = 0;
	...
	for(x = 0; x < 5; x++, gid += HIDDENLINES)
	{
		for(y = 0; y < 20 - HIDDENLINES; y++, a++)
		{
			...
			menugads[a].Height = 8;
			menugads[a].Flags = GFLG_GADGHCOMP;
			menugads[a].Activation = GACT_RELVERIFY;
			menugads[a].GadgetType = GTYP_BOOLGADGET;
	...
	IDOpus->AddGadgetBorders(&menukey, &menuslidergads[1], 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	...
	for(;;)
	{
		...
		while(IMsg = getintuimsg())
		{
				...
				case 0:
					...
					case 1:
						if(b = doload(CFG_MENU, a))
						{
							showmenus(5);
							if(a)
								dogadgetinfo(cfg_string[(b == 1) ? STR_NEW_MENUS_LOADED : STR_MENU_MAX_REACHED]);
							else
								dogadgetinfo(cfg_string[(b == 1) ? STR_DEFAULT_MENUS_LOADED : STR_MENU_MAX_REACHED]);
						}
						break;
					}
					break;
			...
			case IDCMP_GADGETUP:
				...
				if(gadgetid > 199)
				{
					if((a = gadgetid - 200) < 0 || a > 4)
						break;
					if(mode == MENU_DELETEMENU || mode == MENU_SORTMENU || mode == MENU_COPYMENU || mode == MENU_SWAPMENU)
					{
						gadgetid = 100 + (a * 20);
					}
					else
				...
				if(gadgetid > 99 && gadgetid < 200)
				{
					...
					if(mode > -1)
					{
						...
						case MENU_DELETEITEM:
							...
							if(((a + 1) % 20) != 0)
							{
								x = ((a + 20) / 20) * 20 - 1;
								y = x - a;
								IExec->CopyMem((char *)&config->menu[a + 1], &config->menu[a], y * sizeof(struct dopusfunction));
						...
						case MENU_INSERTITEM:
							...
							for(b = x - 1; b >= a; b--)
								IExec->CopyMem((char *)&config->menu[b], &config->menu[b + 1], sizeof(struct dopusfunction));
							clearmenu(&config->menu[a]);
							...
							else
							{
								...
								else
								{
									...
									case MENU_COPYITEM:
										...
										IExec->CopyMem((char *)selmenu, (char *)secondmenu, sizeof(struct newdopusfunction));
									...
									case MENU_COPYMENU:
										if(omnum < 0 || mnum == omnum)
											break;
										IExec->CopyMem((char *)&config->menu[omnum * 20], (char *)&config->menu[mnum * 20], sizeof(struct newdopusfunction) * 20);
										b = (mnum * 20) + 20;
										for(a = mnum * 20; a < b; a++)
										{
									...
									case MENU_SWAPMENU:
										if(omnum < 0 || mnum == omnum)
											break;
										SwapMem((char *)&config->menu[omnum * 20], (char *)&config->menu[mnum * 20], sizeof(struct newdopusfunction) * 20);
					...
					else
					{
						...
						else
						{
							...
							showconfigscreen(CFG_MENU);
							IDOpus->FixSliderPot(Window, &menuslidergads[0], menuoffset, 20, 20 - HIDDENLINES, 2);
							selmenu = NULL;
							showmenus(7);
						}
					}
				...
				case MENU_OKAY:
					doundo(undo, 0);
					IIntuition->RemoveGList(Window, menugads, MENUCOUNT);
					IDOpus->LFreeRemember(&menukey);
					menugads = NULL;
					IIntuition->ClearMenuStrip(Window);
					projectmenu.NextMenu = NULL;
					IDOpus->FSSetMenuStrip(Window, &projectmenu);
			...
			case IDCMP_GADGETDOWN:
				...
				case MENU_SLIDER:
					a = menuoffset;
					menuoffset = IDOpus->GetSliderPos(&menuslidergads[0], 20, 20 - HIDDENLINES);
					if(a != menuoffset)
						showmenus(1);
					for(;;)
					{
					...
					IDOpus->ShowSlider(Window, &menuslidergads[0]);
					break;
				case MENU_MOVEUP:
					menuoffset -= 2;
					if(menuoffset < -1)
...
void showmenuname(struct newdopusfunction *menu, struct Gadget *gadget)
{
	int x, y;
	...
	if(menu)
	{
		...
		if(menu == selmenu)
			IDOpus->HiliteGad(gadget, rp);
	}
	else
	{
		IGraphics->SetAPen(rp, screen_pens[1].pen);
		IGraphics->RectFill(rp, x, y, x + 111, y + 7);
	}
}

void showmenutext(STRPTR txt, int x, int y, int fp, int bp)
{
	int a, l;

	l = (14 * 8) / rp->Font->tf_XSize;
	IGraphics->SetAPen(rp, screen_pens[fp].pen);
	IGraphics->SetBPen(rp, screen_pens[bp].pen);
	IGraphics->Move(rp, x, y + 6);
	if(txt)
	{
		if((a = strlen(txt)) > 0)
		{
			if(a > l)
				a = l;
			IGraphics->Text(rp, txt, a);
		}
	}
	else
		a = 0;
	if(a < l)
		IGraphics->Text(rp, spacestring, l - a);
	IGraphics->SetAPen(rp, screen_pens[bp].pen);
	IGraphics->RectFill(rp, x + (l * rp->Font->tf_XSize), y, x + 111, y + 7);
	IGraphics->SetBPen(rp, screen_pens[0].pen);
}
...
void do_menuscreen_title(int title)
{
	if(!paint_state)
		doscreentitle(cfg_string[title]);
	else
	{
		char temp[80];

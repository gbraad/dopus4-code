...
void dohelpmsg(STRPTR text)
{
	...
	size = strlen(text);
	if(!(buf = IDOpus->LAllocRemember(&key, size + 1, MEMF_CLEAR)))
		return;
	...
	if(!(helpbuf = IDOpus->LAllocRemember(&key, (lines + 2) * 4, MEMF_CLEAR)))
	{
	...
	if((wind = openwindow(&requestwin)))
	{
		...
		if(IDOpus->AddListView(&helplist, 1))
		{
			IGraphics->SetAPen(wind->RPort, screen_pens[1].pen);
			IDOpus->AddGadgets(wind, &helpcancelgad, editfuncgads, 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
			for(;;)
			{
		      ...
		      endhelp:
			IDOpus->RemoveListView(&helplist, 1);
		}
...
void load_clips()
{
	...
	if(!(file = IDOS->Open("t:configopus.clip", MODE_OLDFILE)))
		return;

	for(;;)
	{
		if((IDOS->Read(file, (char *)&clip, sizeof(struct Clip))) < sizeof(struct Clip))
			break;
		if(clip.func.function && (funcbuf = IDOpus->LAllocRemember(&clipkey, (int)clip.func.function, 0)))
		{
			if((IDOS->Read(file, funcbuf, (int)clip.func.function)) < (int)clip.func.function)
				break;
		}
		else
			funcbuf = NULL;
...
void save_clips()
{
	...
	if(!(file = IDOS->Open("t:configopus.clip", MODE_NEWFILE)))
		return;
	...
	while(clip)
	{
		...
		if((IDOS->Write(file, (char *)clip, sizeof(struct Clip))) < sizeof(struct Clip))
			break;
		if(function && ((IDOS->Write(file, (char *)function, (int)clip->func.function)) < (int)clip->func.function))
			break;
		clip = clip->next;
	}
...
int readfile(STRPTR name, STRPTR *buf, int *size)
{
	...
	if(IDOpus->CheckExist(name, size) >= 0 || !(in = IDOS->Open(name, MODE_OLDFILE)))
		return (-1);
	if(!(*buf = IExec->AllocMem(*size, MEMF_CLEAR)))
	{
		IDOS->Close(in);
		return (-2);
	}
...
void init_strings()
{
	...
	for(a = 0; a < 3; a++)
		fileclasstype[a] = cfg_string[STR_FILECLASS_DEFINE + a];
	fileclasstype[3] = cfg_string[STR_FILECLASS_DUPLICATE];

	for(a = 0; a < 9; a++)
		fontplacenames[a] = cfg_string[STR_FONTPLACE_CLOCK + a];
	...
	for(a = 0; a < 6; a++)
		formatnames[a] = cfg_string[STR_FORMAT_NAME + a];
	if(network)
	{
	...
	for(a = 0; a < 3; a++)
		icontypes[a] = cfg_string[STR_ICONS_DRAWER + a];
	...
	for(a = 0; a < 3; a++)
		systemgads[a] = cfg_string[STR_SYSTEM_AMIGADOS + a];
	...
	for(a = 0; a < 6; a++)
		gadgetgads[a + 2] = cfg_string[STR_GADGET_COPYBANK + a];
	gadgetgads[8] = cfg_string[STR_OKAY];
	gadgetgads[9] = cfg_string[STR_CANCEL];
	gadgetgads[10] = NULL;

	for(a = 0; a < 16; a++)
		gadflaglist[a] = cfg_string[STR_FLAG_AUTOICONIFY + a];
	gadflaglist[16] = NULL;
	...
	for(a = 0; a < 5; a++)
		editfuncgads2[a] = cfg_string[STR_EDIT_NAME + a];
	...
	for(a = 0; a < 8; a++)
		menugadgets[a] = cfg_string[STR_MENU_COPYMENU + a];
	...
	for(a = 0; a < 3; a++)
		drivegadgets[a + 1] = cfg_string[STR_GADGET_COPYBANK + a];
	for(a = 0; a < 5; a++)
		drivegadgets[a + 4] = cfg_string[STR_DRIVES_SORTBANK + a];
	...
	for(a = 0; a < 2; a++)
		screengadgets[a + 1] = cfg_string[STR_SCREEN_COLORS + a];
	...
	for(a = 0; a < 3; a++)
		palettegadgets[a] = (char *)-1;
	for(a = 0; a < 3; a++)
		palettegadgets[a + 3] = cfg_string[STR_PALETTE_RED + a];
	palettegadgets[6] = NULL;

	for(a = 0; a < 15; a++)
		coloursgadgets[a] = cfg_string[STR_COLOR_STATUS + a];
	coloursgadgets[15] = NULL;

	for(a = 0; a < 4; a++)
		screenmodegadgets[a] = cfg_string[STR_SCREENMODE_WIDTH + a];
	...
	for(a = 0; a < 3; a++)
		arrowgadtxt[a] = cfg_string[STR_ARROWS_INSIDE + a];
	arrowgadtxt[3] = NULL;
...
//    languagelist.title=cfg_string[STR_LANGUAGE_LIST_TITLE];
	...
	for(a = 0; a < 11; a++)
		classopslist[a] = cfg_string[STR_CLASSOPS_AND + a];
	classopslist[11] = cfg_string[STR_CLASSOPS_MATCHI];
	classopslist[12] = NULL;

	for(a = 0; a < 4; a++)
		palettenames[a] = cfg_string[STR_PALETTE_RESET + a];
	for(a = 0; a < 9; a++)
		palettenames[a + 4] = cfg_string[STR_PALETTE_TINT + a];
...
void setup_list_window(struct NewWindow *win, struct DOpusListView *list, struct Gadget *gadget, int count)
{

					...
					if(drag == 2)
					{
						if(x != ox || y != oy)
						{
							if(scr_gadget_rows && dopus_curgadbank)
							{
								...
								if(oc != comp)
								{
									IGraphics->RemIBob(&drag_bob, main_rp, IIntuition->ViewPortAddress(Window));
									if(comp > -1)
										dragcompgad(comp, 0);
									if(oc > -1 && isvalidgad(&dopus_curgadbank->gadgets[gad]))
									{
										...
										comp = -1;
									IGraphics->AddBob(&drag_bob, main_rp);
								}
							}
							drag_sprite.X = x - offx;
							drag_sprite.Y = y - offy;
							IGraphics->SortGList(main_rp);
							IGraphics->DrawGList(main_rp, IIntuition->ViewPortAddress(Window));
							IGraphics->WaitTOF();
						}
						ox = x;
						oy = y;
						...
						if(fa == la && drag && selectedentry)
						{
							if(x < scrdata_dirwin_xpos[data_active_window] || x > scrdata_dirwin_xpos[data_active_window] + scrdata_dirwin_width[data_active_window])
							{
								...
								if(ptr)
								{
									...
									IGraphics->SetAPen(&drag_bob_rastport, (type <= ENTRY_FILE || (type == ENTRY_DEVICE && (selectedentry->size == DLT_DEVICE || selectedentry->size == DLT_VOLUME))) ? screen_pens[config->filesselfg].pen : screen_pens[config->dirsselfg].pen);
									IGraphics->SetRast(&drag_bob_rastport, (type <= ENTRY_FILE || (type == ENTRY_DEVICE && (selectedentry->size == DLT_DEVICE || selectedentry->size == DLT_VOLUME))) ? screen_pens[config->filesselbg].pen : screen_pens[config->dirsselbg].pen);
									IGraphics->Move(&drag_bob_rastport, 0, scr_font[FONT_DIRS]->tf_Baseline);
									...
									IGraphics->Text(&drag_bob_rastport, &ptr[e], c);
									drag_sprite.X = x - offx;
									drag_sprite.Y = y - offy;
									IGraphics->AddBob(&drag_bob, main_rp);
									IGraphics->SortGList(main_rp);
			...
			if(x >= scrdata_dirwin_xpos[1 - data_active_window] && x <= scrdata_dirwin_xpos[1 - data_active_window] + scrdata_dirwin_width[1 - data_active_window] && y >= scrdata_dirwin_ypos[1 - data_active_window] && y <= scrdata_dirwin_ypos[1 - data_active_window] + scrdata_dirwin_height && drag == 2)
			{
				if(multidrag)
				{
					...
					while(last_selected_entry)
					{
						...
						{
							IIntuition->CurrentTime(&time_previous_sec, &time_previous_micro);
...
//				IDOpus->StrCombine(sbuf, temp->name, ".info", FILEBUF_SIZE - 1);
		...
		if(sel)
		{
			dbclick = IIntuition->DoubleClick(time_previous_sec, time_previous_micro, time_current_sec, time_current_micro);
		}
		else
		{
			dbclick = -1;
...
void dateselect(struct DateStamp *ds1, struct DateStamp *ds2, int boobs, int and)
{
	...
	while(temp)
	{
		if(temp->type != ENTRY_CUSTOM || temp->subtype != CUSTOMENTRY_USER || temp->size & CUSTENTRY_CANSELECT)
		{
			if(!and)
				temp->selected = 0;
			if(!temp->selected && IDOS->CompareDates(&(temp->date), ds1) >= 0 && IDOS->CompareDates(ds2, &(temp->date)) >= 0)
				wildselectthisone(temp, data_active_window, boobs);
		}
		temp = temp->next;
	}

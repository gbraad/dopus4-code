...
void makedir(int rexx)
{
	...
	FOREVER
	{
		...
		if((data = IDOS->ExamineObjectTags(EX_StringName, new_directory, TAG_END)))
		{
			if(win > -1)
			{
				addfile(dopus_curwin[win], win, data->Name, -1, ENTRY_DIRECTORY, &data->Date, NULL, data->Protection, ENTRY_DIRECTORY, TRUE, NULL, NULL, data->OwnerUID, data->OwnerGID);
				seename(win);
		...
		if((config->iconflags & ICONFLAG_MAKEDIRICON) || addicon)
		{
			...
			if((iconwrite(ICONTYPE_DRAWER, new_directory)) == 1 && (data = IDOS->ExamineObjectTags(EX_StringName, new_directory, TAG_END)))
			{
				if(win > -1)
				{
					addfile(dopus_curwin[win], win, data->Name, data->FileSize, ENTRY_FILE, &data->Date, data->Comment, data->Protection, ENTRY_FILE, TRUE, NULL, NULL, data->OwnerUID, data->OwnerGID);
					seename(win);
					update_buffer_stamp(win, 1);
				}
				IDOS->FreeDosObject(DOS_EXAMINEDATA, data);
...
int readfile(STRPTR name, STRPTR *buf, int64 *size)
{
	...
	if((*buf = doAllocVec(filesize, MEMF_ANY)))
	{

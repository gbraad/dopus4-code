...
int getprotectdata(int *prot, int *mask)
{
	...
	for(a = 0; a < 8; a++)
	{
		IExec->CopyMem((char *)protect_bit_template, (char *)protect_bits[a], sizeof(protect_bit_template));
	...
	IDOpus->RefreshRequesterObject(&protect_req, NULL);
	...
	FOREVER
	{
		...
		IExec->Wait(1 << pwindow->UserPort->mp_SigBit);
	}
}
...
void do_prot_display(int bits, Object_Border *border, struct Gadget *gadgets)
{
	...
	IDOpus->ObjectText(&protect_req, border->ob_left, border->ob_top, border->ob_width, border->ob_height, bitbuf, TEXTPOS_CENTER);
	...
	if(gadgets)
	{
		...
		IIntuition->RefreshGList(gadgets, protect_req.rb_window, NULL, 8);
	}
}

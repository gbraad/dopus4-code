...
static struct TagItem search_title[] =
{
...
{
	search_text_gadget, search_case_gadget, search_wild_gadget, search_onlyword_gadget, search_okay_gadget, search_cancel_gadget, NULL
};
...
int get_search_data(STRPTR buffer, int *flagptr, struct Window *window, struct TextFont *font)
{
	...
	IDOpus->AddRequesterObject(&search_req, search_title);
	IDOpus->RefreshRequesterObject(&search_req, NULL);

	IDOpus->ActivateStrGad(gadlist, swindow);

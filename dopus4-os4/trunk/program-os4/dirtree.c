...
void dotree(int win)
{
	...
	if(first_recurse)
	{
		...
		IDOpus->LFreeRemember(&recurse_dir_key);
	}
...
int build_tree(struct RecursiveDirectory *rec)
{
	static int recdepth;
	int a, b;
	...
	while(rec)
	{
		...
		for(a = 0; a < recdepth; a++)
		{
			...
			{
				if(recurse_parent_array[a]->next)
					tree_buffer[b++] = '|';
				else
					tree_buffer[b++] = ' ';
				if(tree_path_buffer)
...
void draw_dirtree_gfx(struct RastPort *rp, int x, int y, int type)
{
	...
	if(status_flags & STATUS_SQUAREPIXEL)
		++x_plus_half;
	...
	case DIRTREEGFX_BRANCH:
		...
		{
			IGraphics->Move(rp, x_plus_half, y + half_ysize);
			IGraphics->Draw(rp, x + scrdata_font_xsize - 1, y + half_ysize);
		}
		break;
	case DIRTREEGFX_CROSS:
	case DIRTREEGFX_FINALCROSS:
		IGraphics->Move(rp, x, y + half_ysize);
		IGraphics->Draw(rp, x + scrdata_font_xsize - ((type == DIRTREEGFX_CROSS) ? 1 : 2), y + half_ysize);
		break;
	}
}

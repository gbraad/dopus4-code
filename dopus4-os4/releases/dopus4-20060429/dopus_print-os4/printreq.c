...
struct TagItem print_filerequester_gadget[] = {
...
void get_print_default(PrintData * printdata);
void show_headerfooter_gadgets(struct Gadget **gadgets, struct Window *window, PrintData * printdata, int headerfooter);
void read_headerfooter_gadgets(struct Gadget **gadgets, struct Window *window, PrintData * printdata, int headerfooter);
int check_print_gadget(struct Window *window, struct Gadget *gad, int min, int max, USHORT code);
...
void do_print(struct VisInfo *vis, char *portname, struct DOpusArgsList *argslist, int argc, char **argv)
{
	...
	for(a = 0; a < 6; a++)
		text_styles[a] = string_table[STR_NORMAL + a];
	for(a = 0; a < 2; a++)
	{
	...
	IDOpus->RefreshRequesterObject(&printreq, NULL);

	print_file_buffer = ((struct StringInfo *)gadgets[PRINT_FILE]->SpecialInfo)->Buffer;
	print_topmargin_buffer = ((struct StringInfo *)gadgets[PRINT_TOPMARGIN]->SpecialInfo)->Buffer;
	print_bottommargin_buffer = ((struct StringInfo *)gadgets[PRINT_BOTTOMMARGIN]->SpecialInfo)->Buffer;
	print_leftmargin_buffer = ((struct StringInfo *)gadgets[PRINT_LEFTMARGIN]->SpecialInfo)->Buffer;
	print_rightmargin_buffer = ((struct StringInfo *)gadgets[PRINT_RIGHTMARGIN]->SpecialInfo)->Buffer;
	...
	if(portname)
	{
		IIntuition->OffGadget(gadgets[PRINT_FILEREQUESTER], window, NULL);
			...
			while(appmsg = (struct AppMessage *)IExec->GetMsg(appport))
			{
				if(appmsg->am_Type == AMTYPE_APPWINDOW)
				{
					if(appmsg->am_NumArgs > 0 && appmsg->am_ArgList[0].wa_Lock)
					{
						...
						IDOpus->TackOn(buf, appmsg->am_ArgList[0].wa_Name, 256);
		...
		while((msg = (struct IntuiMessage *)IExec->GetMsg(window->UserPort)))
		{
			...
			case IDCMP_GADGETDOWN:
				...
				case OUTPUT_PRINTER:
					...
					{
						gadgets[OUTPUT_PRINTER]->Flags |= GFLG_SELECTED;
						IIntuition->RefreshGList(gadgets[OUTPUT_PRINTER], window, NULL, 1);
					}
					gadgets[OUTPUT_FILE]->Flags &= ~GFLG_SELECTED;
					IIntuition->RefreshGList(gadgets[OUTPUT_FILE], window, NULL, 1);
			...
			case IDCMP_GADGETUP:
				...
				case PRINT_TOPMARGIN:
					if(check_print_gadget(window, gadgets[PRINT_TOPMARGIN], 1, atoi(print_bottommargin_buffer) - 5, code))
						refgad = (struct Gadget *)iaddress;
					printdata->top_margin = atoi(print_topmargin_buffer);
					break;
				case PRINT_BOTTOMMARGIN:
					if(check_print_gadget(window, gadgets[PRINT_BOTTOMMARGIN], atoi(print_topmargin_buffer) + 5, 999, code))
						refgad = (struct Gadget *)iaddress;
					printdata->bottom_margin = atoi(print_bottommargin_buffer);
					break;
				case PRINT_LEFTMARGIN:
					if(check_print_gadget(window, gadgets[PRINT_LEFTMARGIN], 1, atoi(print_rightmargin_buffer) - 1, code))
						refgad = (struct Gadget *)iaddress;
					printdata->left_margin = atoi(print_leftmargin_buffer);
					break;
				case PRINT_RIGHTMARGIN:
					if(check_print_gadget(window, gadgets[PRINT_RIGHTMARGIN], atoi(print_leftmargin_buffer) + 1, 999, code))
						refgad = (struct Gadget *)iaddress;
					printdata->right_margin = atoi(print_rightmargin_buffer);
					break;

				case PRINT_TABSIZE:
					if(check_print_gadget(window, gadgets[PRINT_TABSIZE], 1, 10, code))
						IDOpus->ActivateStrGad(gadgets[PRINT_TOPMARGIN], window);
				...
				case HEADFOOT_TITLESTRING:
					IDOpus->RefreshStrGad(gadgets[HEADFOOT_TITLESTRING], window);
					break;
				...
				case OUTPUT_FILESTRING:
					...
					if(gadgets[OUTPUT_PRINTER]->Flags & GFLG_SELECTED)
					{
						gadgets[OUTPUT_PRINTER]->Flags &= ~GFLG_SELECTED;
						gadgets[OUTPUT_FILE]->Flags |= GFLG_SELECTED;
						IIntuition->RefreshGList(gadgets[OUTPUT_PRINTER], window, NULL, 2);
						printdata->print_flags |= PRINTFLAG_FILE;
					}
					break;
				...
				if(refgad)
				{
					IDOpus->RefreshStrGad(refgad, window);
					if(code != 0x9)
						activate_next_gadget(refgad, window);
				}
				if(gadgetid != PRINT_EXIT)
					break;
...
int check_print_gadget(struct Window *window, struct Gadget *gad, int min, int max, USHORT code)
{
	if(IDOpus->CheckNumGad(gad, window, min, max))
	{
		IIntuition->DisplayBeep(NULL);
		if(code != 0x9)

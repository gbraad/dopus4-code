...
printfile(struct RequesterBase *reqbase, char *filename, PrintData *printdata, struct Requester *requester)
{
	...
	for(;;)
	{
		if(IDOpus->CheckExist(filename, &filesize) < 0)
			break;
		if(filesize == 0)
			return 0;
	...
	if(!(datetime = IDOpus->LAllocRemember(&memkey, sizeof(struct DateTime), MEMF_CLEAR)) || !(handle = IDOpus->LAllocRemember(&memkey, sizeof(struct PrintHandle), MEMF_CLEAR)))
	{
	...
	{
		if(!(marginbuf = IDOpus->LAllocRemember(&memkey, margin + 1, MEMF_CLEAR)))
			goto ENDPRINT;
		for(a = 0; a < margin; a++)
			marginbuf[a] = ' ';
	}
	if(!(linebuffer = IDOpus->LAllocRemember(&memkey, (handle->paper_width * 2) + 4, MEMF_CLEAR)))
		goto ENDPRINT;

	if(IIntuition->Request(requester, reqbase->rb_window))
	{
		font = reqbase->rb_window->RPort->Font;
		handle->req_rp = requester->ReqLayer->rp;
		handle->req_width = requester->Width;
		IDOpus->Do3DBox(handle->req_rp, 2, 1, requester->Width - 4, requester->Height - 2, reqbase->rb_shine, reqbase->rb_shadow);
		IGraphics->SetAPen(handle->req_rp, reqbase->rb_fg);
		IGraphics->SetBPen(handle->req_rp, reqbase->rb_bg);
		IGraphics->SetDrMd(handle->req_rp, JAM2);
		IGraphics->SetFont(handle->req_rp, font);
		handle->progress_y = font->tf_YSize * 3;
		print_status(handle, (printdata->print_flags & PRINTFLAG_FILE && printdata->output_file[0]) ? string_table[STR_CREATING_FILE] : string_table[STR_OPENING_PRINTER], handle->progress_y);
	}
	...
	if(printdata->print_flags & PRINTFLAG_FILE && printdata->output_file[0])
	{
		...
		a = LOCK_DIFFERENT;
		src_lock = IDOS->Lock(filename, ACCESS_READ);
		dst_lock = IDOS->Lock(printdata->output_file, ACCESS_READ);
		if(src_lock && dst_lock)
			a = IDOS->SameLock(src_lock, dst_lock);
		IDOS->UnLock(dst_lock);
		IDOS->UnLock(src_lock);
		if(a == LOCK_SAME)
		{
			goto ENDPRINT;
		}
	...
	else
	{
		...
		for(;;)
		{
			if(!(IExec->OpenDevice("printer.device", 0, (struct IORequest *)handle->ioreq, 0)))
				break;
		...
		if(printdata->print_pitch == PITCH_PICA)
			prefs->PrintPitch = PICA;
		else if(printdata->print_pitch == PITCH_FINE)
			prefs->PrintPitch = FINE;
		else if(printdata->print_pitch == PITCH_ELITE)
			prefs->PrintPitch = ELITE;

		if(printdata->text_quality == QUALITY_DRAFT)
			prefs->PrintQuality = DRAFT;
		else
	...
	if(handle->req_rp)
	{
		IGraphics->SetAPen(handle->req_rp, reqbase->rb_bg);
		IGraphics->RectFill(handle->req_rp, 2, 1, requester->Width - 3, requester->Height - 2);
	...
	if(!(fileh = IDOS->Open(filename, MODE_OLDFILE)))
		goto ENDPRINT;
	size = IDOS->Read(fileh, buffer, buffersize);

	handle->filesize = filesize;

	if(handle->req_rp)
	{
		print_status(handle, string_table[STR_PRINTING_FILE], font->tf_YSize);
		print_status(handle, string_table[STR_PRESS_ESCAPE], (font->tf_YSize * 5));
	...
	for(;;)
	{
		if(bufpos >= size && (!fileh || (size = IDOS->Read(fileh, buffer, buffersize)) < 1))
		{
			if(pos > 0)
			{
				if((marginbuf && !(print_str(handle, marginbuf, -1))) || !(print_str(handle, linebuffer, pos)))
					goto ENDPRINT;
			}
			...
			if(handle->current_line > 0 && printdata->print_flags & PRINTFLAG_EJECT && !(print_str(handle, "\f", 1)))
				break;
			break;
		}
		bufpos = 0;
		while(bufpos < size)
		{
			...
			if(handle->current_line < handle->paper_lines)
			{
				...
				if(handle->current_line == 0 && printdata->headfoot[HEADER].headfoot_flags)
				{
					if(!(do_header_footer(handle, printdata, HEADER)))
						goto ENDPRINT;
				...
				else
				{
					while(bufpos < size && handle->current_line < handle->paper_lines)
					{
						...
						case '\t':
							if((pos % printdata->tab_size) == 0)
								a = printdata->tab_size;
							else
								a = (((pos / printdata->tab_size) + 1) * printdata->tab_size) - pos;
						...
						default:
							if(isspace(buffer[bufpos]))
								lastspace = pos;
							else if(ispunct(buffer[bufpos]))
							{
								if(lastspace == -1 || pos > lastspace + 8)
									lastspace = pos;
							}
							...
							if(pos > handle->paper_width)
							{
								...
								if(lastspace > 0 && lastspace < pos)
								{
									...
									pos -= a;
									if(pos > 0)
										IExec->CopyMem(&linebuffer[a], linebuffer, pos);
								}
								else
								{
...
do_header_footer(struct PrintHandle *handle, PrintData *printdata, int type)
{
	...
	if(printdata->headfoot[type].headfoot_flags)
	{
		...
		for(a = 0; a < sizeof(handle->buffer); a++)
			handle->buffer[a] = ' ';

		if(printdata->headfoot[type].headfoot_flags & HEADFOOTFLAG_DATE)
			IExec->CopyMem(handle->datebuf, &handle->buffer[leftoffset + 1], 9);
		...
		if(printdata->headfoot[type].headfoot_flags & HEADFOOTFLAG_TITLE)
		{
			...
			if((a = strlen(titlebuf)) > handle->paper_width)
			{
				...
				b = (handle->paper_width - a) / 2;
			b += leftoffset;
			IExec->CopyMem(titlebuf, &handle->buffer[b], a);
		}
		...
		if(!(print_str(handle, handle->buffer, len)))
			return (0);
...
do_printstyle(struct PrintHandle *handle, int style, int turnon)
{
	...
	{
		if(turnon)
			a = print_str(handle, esc_styles[style], -1);
		else
		{
...
printer_command(struct RequesterBase *reqbase, struct PrintHandle *handle, char *command)
{
	...
	for(;;)
	{
		if(!(IExec->DoIO((struct IORequest *)handle->ioreq)))
			break;
		if(!(check_error(reqbase, string_table[STR_UNABLE_TO_OPEN_PRINTER], 0)))
			return (0);
	}
...
void show_progress(struct PrintHandle *handle)
{
	float percent;
	char buf[20];

	if(!handle->total_pos || !handle->filesize)
		percent = 0;
	else
...
void print_status(struct PrintHandle *handle, char *text, int y)
{
	...
	{
		IGraphics->SetAPen(handle->req_rp, reqbase->rb_bg);
		IGraphics->RectFill(handle->req_rp, 2, y, x, y + font->tf_YSize);
	}
	IGraphics->SetAPen(handle->req_rp, reqbase->rb_fg);
	IDOpus->UScoreText(handle->req_rp, text, x, y + font->tf_Baseline, -1);

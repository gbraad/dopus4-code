...
void allocdirbuffers(int newnum)
{
	int win, a, num;
	...
	for(win = 0; win < 2; win++)
	{
		...
		{
			dir = dopus_firstwin[win];
			for(a = 0; a < data_buffer_count[win] - 1; a++)
				dir = dir->next;
...
int bringinbuffer(char *dirbuf, int win, int read)
{
	int a, otherwin;
...
void startgetdir(int win, int flags)
{
	checkcurrentbuffer(win);
...
void copydirwin(struct DirectoryWindow *sourcewin, struct DirectoryWindow *destwin, int dest)
{
...
void swapdirwin()
{
	...
	IExec->CopyMem((char *)dopus_curwin[0], (char *)&tempwin, sizeof(struct DirectoryWindow));
	tempwin.next = dopus_curwin[1]->next;
	tempwin.number = dopus_curwin[1]->number;
	tempwin.last = dopus_curwin[1]->last;
	dopus_curwin[1]->next = dopus_curwin[0]->next;
	dopus_curwin[1]->number = dopus_curwin[0]->number;
	dopus_curwin[1]->last = dopus_curwin[0]->last;
	IExec->CopyMem((char *)dopus_curwin[1], (char *)dopus_curwin[0], sizeof(struct DirectoryWindow));
	IExec->CopyMem((char *)&tempwin, (char *)dopus_curwin[1], sizeof(struct DirectoryWindow));
...
void advancebuf(int win, int dir)
{
	int a;
	...
	if(!checkcurrentbuffer(win))
	{
		...
		{
			for(a = 0; a < dir; a++)
				dopus_curwin[win] = dopus_curwin[win]->next;
		}
		else
		{
			for(a = dir; a < 0; a++)
				dopus_curwin[win] = dopus_curwin[win]->last;
		}
	}
}
...
struct DirectoryWindow *findbuffer(char *dirbuf, int win, int canchecklocks, int onlyreturn)
{
	...
	for(try = 0; try < 2; try++)
	{
		if(checklocks && !(lock = IDOS->Lock(tempbuf, ACCESS_READ)))
			break;
		...
		for(a = 0; a < data_buffer_count[win]; a++, dir = dir->last)
		{
			if(dir->directory[0])
			{
				if(checklocks && (testlock = IDOS->Lock(dir->directory, ACCESS_READ)))
				{
					if(IDOS->SameLock(lock, testlock) == LOCK_SAME)
						founddir = 1;
...
int replacepart(char *string, char *old, char *new)
{
...
void dolistbuffers(int destwin)
{
	int a, b, c, win;
	struct DateStamp ds;
	struct DirectoryWindow *dir;
	...
	busy();
	b = 0 /*scrdata_dispwin_nchars[destwin] */ ;
	name[0] = 0;
	IDOS->DateStamp(&ds);
	for(win = 0; win < 2; win++)
	{
		...
		for(a = 0; a < data_buffer_count[win]; a++)
		{
			if(dir->directory[0])
			{
				addfile(dopus_curwin[destwin], destwin, name, 0, ENTRY_CUSTOM, &ds, dir->directory, 0, CUSTOMENTRY_BUFFERLIST, FALSE, dir->directory, NULL, 0, 0);
				if((c = strlen(dir->directory)) > b)
					b = c;
			}
			dir = dir->next;
		}
...
int checkcurrentbuffer(int win)
{
...
void userentrymessage(struct DirectoryWindow *dir, struct Directory *entry, int type)
{
	...
	if(!(msg = IRexxSys->CreateRexxMsg(general_port, NULL, str_arexx_portname)))
		fail = 1;
	else
	{
		...
		if(!(IRexxSys->FillRexxMsg(msg, 4, 9)))
			fail = 1;
	}
	...
	IExec->Forbid();
	if(!(port = IExec->FindPort(dir->custhandler)))
	{
		char message[200];
	...
	FOREVER
	{
		...
		if((reply = (struct RexxMsg *)IExec->GetMsg(general_port)))
		{
			IRexxSys->ClearRexxMsg(msg, 4);
			IRexxSys->DeleteRexxMsg(msg);
			break;
...
void makespecialdir(int win, char *title)
{

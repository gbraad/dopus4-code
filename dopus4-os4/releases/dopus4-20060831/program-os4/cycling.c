...
void colourcycle()
{
	...
	{
		if(!cy_WeAreWaiting)
			IGraphics->LoadRGB4(cy_VPort, cy_UndoBuffer, cy_ColNum);
	...
	for(i = 0; i < cy_RangeNum; i++)
	{
		...
		if(cy_TicksPassed[i] == cy_Range[i].rate)
		{
			...
			IGraphics->LoadRGB4(cy_VPort, cy_ColMap, cy_ColNum);
		}
	}
}

int initcycle(struct ViewPort *ViewPort, UWORD *ColMap, int ColNum, CRange *Range, int RangeNum)
{
	short i;

	if(!ViewPort || !ColMap || !ColNum || ColNum > 64 || !Range || !RangeNum)
		return (FALSE);
	...
	IExec->AddIntServer(INTB_VERTB, &cy_VBlank);
	cy_CodeRun = TRUE;
	return (TRUE);
}
...
void stopcycle()
{
	...
	cy_WeAreWaiting = TRUE;
	cy_Command = CY_WAIT;
	IExec->RemIntServer(INTB_VERTB, &cy_VBlank);
	for(i = 0; i < cy_ColNum; i++)
		cy_ColMap[i] = cy_UndoBuffer[i];
	IGraphics->WaitTOF();
	IGraphics->WaitTOF();

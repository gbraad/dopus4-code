...
void dofontdemo(STRPTR name, int size)
{
	...
	clip.MaxX = x_off + 611;
	clip.MaxY = y_off + 167;
	IDOpus->Do3DBox(rp, x_off + 146, y_off + 93, 468, 76, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	IGraphics->SetAPen(rp, screen_pens[0].pen);
	IGraphics->RectFill(rp, x_off + 146, y_off + 93, x_off + 613, y_off + 167);
	...
	if(font = getfont(name, &size, 0))
	{
		...
		{
			if(isupper(buf[0][a]))
				buf[1][a] = IUtility->ToLower(buf[0][a]);
			else
				buf[1][a] = IUtility->ToUpper(buf[0][a]);
		}
		a = 0;
		while(y < 76 + font->tf_Baseline)
		{
			IGraphics->Move(rp, x_off + 148, y + y_off + 94);
			IGraphics->Text(rp, buf[a], l);
			a = a ? 0 : 1;
			y += font->tf_YSize;
		}
		if(reg)
		...
		IDOpus->UScoreText(rp, cfg_string[STR_FONT_COULD_NOT_BE_LOADED], x_off + 289, y_off + 133, -1);
	unbusy();
}

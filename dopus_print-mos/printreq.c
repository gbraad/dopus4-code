...
void do_print(struct VisInfo *vis, char *portname, struct DOpusArgsList *argslist, int argc, char **argv)
{
	...
	if(!(printdata = AllocMem(sizeof(PrintData), MEMF_CLEAR)))
		return;
	...
	for(a = 0; a < 2; a++)
	{
		if(window = OpenRequester(&printreq))
			break;
	...
	if(!(gadgets = LAllocRemember(&printreq.rb_memory, sizeof(struct Gadget *) * PRINT_NUM_GADGETS, 0)) || !(addreqgadgets(&printreq, print_gadgets, gadgets)) || !(AddRequesterObject(&printreq, print_output_text)))
	{
		CloseRequester(&printreq);
		FreeMem(printdata, sizeof(PrintData));
		return;
	...
	if(!portname && WorkbenchBase && (appport = CreateMsgPort()))
		...
		while((msg = (struct IntuiMessage *)GetMsg(window->UserPort)))
		{
			...
			case IDCMP_GADGETUP:
				...
				case HEADFOOT_TEXTSTYLE:
					...
					DoCycleGadget(gadgets[HEADFOOT_TEXTSTYLE], window, text_styles, printdata->headfoot[headerfooter].text_style);
					break;
...
void show_headerfooter_gadgets(struct Gadget **gadgets, struct Window *window, PrintData *printdata, int headerfooter)
{
	DoCycleGadget(gadgets[PRINT_HEADERFOOTER], window, header_footer_text, headerfooter);
	DoCycleGadget(gadgets[HEADFOOT_TEXTSTYLE], window, text_styles, printdata->headfoot[headerfooter].text_style);

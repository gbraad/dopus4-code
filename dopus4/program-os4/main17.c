...
int rexxdisp(struct RexxMsg *msg, struct CommandList *cmd, char *command)
{
	...
	buf[0] = 0;
	if(cmd)
		func_global_function = cmd->function;
	for(a = 0; a < 16; a++)
	{
		rexx_args[a][0] = 0;
		rexx_arg_value[a] = 0;
	...
	if(!command || !command[0])
		...
		rexx_argcount = parse(command);

	for(a = 0; a < rexx_argcount; a++)
	{
		for(b = 0; b < 256; b++)
		{
			if((ch = rexx_args[a][b]) == 0)
				break;
			if(ch == 160)
				rexx_args[a][b] = ' ';
		}
	...
	if(rexx_argcount == 0 && func_global_function != FUNC_PRINT)
		rexx_global_flag = 0;
	else
	{
		rexx_global_flag = 1;
		...
		case FUNC_PROTECT:
			...
			if(rexx_argcount > 2)
				rexx_arg_value[1] = getprotval(rexx_args[2]) ^ 15;
			else
				rexx_arg_value[1] = 0;
			return (0);
		case FUNC_SELECT:
	...
	case FUNC_SETVAR:
		...
		else
		{
			...
			{
				rexx_set_return(msg, IDOS->IoErr(), NULL);
			}
		}
		break;
	...
	case FUNC_DISKINFO:
		if(rexx_argcount < 1)
			ptr = str_pathbuffer[data_active_window];
		else
			ptr = rexx_args[0];
		dodiskinfo(ptr);
		break;
	...
	case FUNC_NOTIFY:
		if(rexx_argcount)
		{
			...
			command = globstring[STR_REALLY_PROCEED];
		else
			break;
	...
	case FUNC_GETSTRING:
		buf[0] = 0;
		if(rexx_argcount == 0)
			command = globstring[STR_ENTER_A_STRING];
		else
		{
			command = rexx_args[0];
			if(rexx_argcount > 1)
	...
	case FUNC_GETSELECTEDDIRS:
		...
		case FUNC_GETSELECTEDALL:
			...
			break;
		case FUNC_GETSELECTEDFILES:
			function = FUNC_GETFILES;
			d = 1;
			break;
		case FUNC_GETSELECTEDDIRS:
			function = FUNC_GETDIRS;
			d = 1;
			break;
		}
		...
		while(entry)
		{
			if((function == FUNC_GETALL || (function == FUNC_GETFILES && entry->type <= ENTRY_FILE) || (function == FUNC_GETDIRS && entry->type >= ENTRY_DIRECTORY)) && (!d || entry->selected))
			{
				if(entry->type != ENTRY_CUSTOM || entry->subtype != CUSTOMENTRY_USER)
					a += strlen(entry->name) + val;
				else if(entry->size & CUSTENTRY_CANSELECT)
					a += 9 + val;
			}
			entry = entry->next;
		}
		...
		while(entry)
		{
			if(entry->selected)
			{
				...
				if(entry->type == ENTRY_CUSTOM)
					ptr = (entry->dispstr ? entry->dispstr : entry->comment);
				if(!ptr)
					ptr = entry->name;
	...
	case FUNC_STATUS:
		...
		case RXSTATUS_CURRENT_DIRECTORY:
			if(f)
			{
				if(!(lock = IDOS->Lock(buf3, ACCESS_READ)))
					break;
				IDOS->UnLock(IDOS->CurrentDir(lock));
				break;
			}
			if(expand_path("", buf))
				buf1 = buf;
		...
		case RXSTATUS_BUFFER_DISPLAYED:
			...
			{
				if(!dirwin || dirwin == dopus_curwin[c])
					break;
				dirwin = dirwin->next;
			}
			if(dirwin)
...
//				D(bug("buf3 = \"%s\"\n", buf3));
		...
		case RXSTATUS_FIRST_NODE:
			...
			{
				if(c == 0)
					dopus_firsttype = (struct dopusfiletype *)val;
				else if(c == 1)
				{
					...
					dopus_firsthotkey = (struct dopushotkey *)val;
			}
			else
			{
				if(c == 0)
		...
		case RXSTATUS_CURRENT_BANK_NUMBER:
			...
			for(a = 0; bank && bank != dopus_curgadbank; a++)
				bank = bank->next;
	...
	case FUNC_FILEINFO:
		...
		if((entry = findfile(dopus_curwin[win], rexx_args[0], NULL)))
		{
			...
			sprintf(buf, "%s%lc%ld%lc%ld%lc%ld%lc%ld%lc%ld%lc%ld%lc%s%lc%s%lc", entry->name, pad[0], (int)entry->size, pad[0], entry->subtype, pad[0], entry->type, pad[0], entry->selected, pad[0], entry->date.ds_Days, pad[0], (entry->date.ds_Minute * 60) + (entry->date.ds_Tick / 50), pad[0], entry->comment, pad[0], buf2, pad[0]);
			rexx_set_return(msg, 0, buf);
			return (1);
		}
		break;
	...
	case FUNC_SELECTENTRY:
		...
		if(!entry || (entry->type == ENTRY_CUSTOM && entry->subtype == CUSTOMENTRY_USER && !(entry->size & CUSTENTRY_CANSELECT)))
		{
			rexx_return(msg, -1);
			return (1);
		}
	...
	case FUNC_ADDCUSTENTRY:
		...
		else
		{
			...
			if(rexx_argcount > 3)
			{
				...
				c |= 0x0000ff00;
			if(rexx_args[0][0] && rexx_argcount > 4 && atoi(rexx_args[4]))
				c |= CUSTENTRY_CANSELECT;
			entry = NULL;
			if(rexx_argcount > 6)
			{
				if((f = atoi(rexx_args[6])) > -1)
				{
					...
					if(entry->last)
						...
						entry = (struct Directory *)-1;
				}
			}
			if((a = (int)addfile(dopus_curwin[data_active_window], data_active_window, buf2, c, ENTRY_CUSTOM, &ds, rexx_args[0], d, CUSTOMENTRY_USER, (rexx_argcount > 5) ? atoi(rexx_args[5]) : 0, NULL, entry, 0, 0)))
			{
				if(b > dopus_curwin[data_active_window]->hlen)
					dopus_curwin[data_active_window]->hlen = b;
			}
	...
	case FUNC_QUERY:
		...
		if(function == FUNC_QUERY)
		{
			...
			case MOD_LISTFORMAT:
				...
				for(a = 0; a <= DISPLAY_LAST; a++)
				{
					if(config->displaypos[b][a] == -1)
						break;
					sprintf(buf2, "%ld ", config->displaypos[b][a]);
					strcat(buf, buf2);
				}
				retbuf = buf;
				break;
			...
			case MOD_FONT:
				if(b < 0 || b >= FONT_LAST)
					break;
				sprintf(buf, "%s/%ld", config->fontbufs[b], config->fontsizes[b]);
				retbuf = buf;
				break;
			...
			case MOD_DISPLAYLENGTH:
				...
				if(b < 0 || b > 1)
					b = data_active_window;
				sprintf(buf, "%ld %ld %ld %ld %ld", config->displaylength[b][DISPLAY_NAME], config->displaylength[b][DISPLAY_COMMENT], config->displaylength[b][DISPLAY_FILETYPE], config->displaylength[b][DISPLAY_OWNER], config->displaylength[b][DISPLAY_GROUP]);
				retbuf = buf;
				break;
			...
			case MOD_WINDOWXY:
				if(!status_iconified && status_publicscreen && Window)
					sprintf(buf, "%ld %ld", Window->LeftEdge, Window->TopEdge);
				else
					sprintf(buf, "%ld %ld", config->wbwinx, config->wbwiny);
				retbuf = buf;
				break;
			...
			case MOD_WINDOWSIZE:
				if(!status_iconified && status_publicscreen && Window)
					...
					sprintf(buf, "%ld %ld", config->scr_winw, config->scr_winh);
				retbuf = buf;
				break;
			...
			case MOD_WINDOWXYWH:
				if(!status_iconified && status_publicscreen && Window)
				{
					sprintf(buf, "%ld %ld %ld %ld", Window->LeftEdge, Window->TopEdge, Window->Width, Window->Height);
				}
				else
				{
					sprintf(buf, "%ld %ld %ld %ld", config->wbwinx, config->wbwiny, config->scr_winw, config->scr_winh);
				}
				retbuf = buf;
				break;
			...
			if(retbuf)
				rexx_set_return(msg, 0, retbuf);
			else
				rexx_return(msg, retval);
			return (1);
		}
		...
		case MOD_LISTFORMAT:
			...
			for(a = 0; a <= DISPLAY_LAST; a++)
			{
				if(rexx_argcount <= a + 2)
					config->displaypos[b][a] = -1;
				else
					config->displaypos[b][a] = atoi(rexx_args[a + 2]);
			}
			break;
		...
		case MOD_DISPLAYLENGTH:
			...
			for(a = 0; a < 6; a++)
			{
				...
				if(config->displaylength[b][displenmap[a]] < 20)
					config->displaylength[b][displenmap[a]] = 20;
				else if(config->displaylength[b][displenmap[a]] > MAX_DISPLAYLENGTH)
					config->displaylength[b][displenmap[a]] = MAX_DISPLAYLENGTH;
			}
			break;
		...
		case MOD_WINDOWXY:
			{
				...
				else
				{
					...
					if(rexx_argcount > 2 && (b = atoi(rexx_args[2])) > -1)
						y = config->wbwiny = b;
					else if(Window)
						y = config->wbwiny = Window->TopEdge;
					else
						y = config->wbwiny;
				}
				if(!status_iconified && status_publicscreen && Window)
				{
					...
					if(y < 0)
						y = 0;
					IIntuition->MoveWindow(Window, x - Window->LeftEdge, y - Window->TopEdge);
				}
				break;
			}
		case MOD_WINDOWSIZE:
			{
				...
				else
				{
					...
					if(rexx_argcount > 2 && (b = atoi(rexx_args[2])) > -1)
						...
						h = config->scr_winh = Window->Height;
					else
						h = config->scr_winh;
				}
				if(!status_iconified && status_publicscreen && Window)
				{
					...
					if(w < scr_min_width)
						w = scr_min_width;
					else if(w > Window->WScreen->Width)
						w = Window->WScreen->Width;
					if(h < scr_min_height)
						h = scr_min_height;
					else if(h > Window->WScreen->Height)
					...
					{
						IIntuition->MoveWindow(Window, dx, dy);
						IIntuition->SizeWindow(Window, w - Window->Width, h - Window->Height);
		...
		case MOD_WINDOWXYWH:
			...
			if(!status_iconified && status_publicscreen && Window)
				IIntuition->ChangeWindowBox(Window, config->wbwinx, config->wbwiny, config->scr_winw, config->scr_winh);
			break;
...
int checkkeyword(char **keywords, int num, int rem)
{
	int a;
...
void removeargstring(int num)
{
	int a;

	for(a = num + 1; a < 16; a++)
	{
		IExec->CopyMem(rexx_args[a], rexx_args[a - 1], 256);
		rexx_args[15][0] = 0;
	}
}
	...
	for(a = 1; a < rexx_argcount; a++)
	{
		...
		else if(rexx_args[a][0] == '-')
		{
			*(flag) &= ~(atoi(&rexx_args[a][1]));
			...
			(*flag) = atoi(rexx_args[a]);
	}
}

void rexx_return(struct RexxMsg *msg, long long num)
{
	char numbuf[24];

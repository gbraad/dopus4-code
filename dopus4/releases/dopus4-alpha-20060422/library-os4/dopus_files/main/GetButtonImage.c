...
struct Image * _DOpus_GetButtonImage(struct DOpusIFace *Self, int w, int h, int fg, int bg, int fpen, int bpen, struct DOpusRemember **key)
{
	...
	IGraphics->RectFill(&trp, 0, 0, w - 1, h - 1);
	Self->DrawRadioButton(&trp, 2, 1, w - 4, h, (fpen != bpen) ? bg : fg, (fpen != bpen) ? fg : bg);
	if((w > 10) && (h > 4) && (fpen != bpen))
	{
		IGraphics->SetAPen(&trp, fpen);
		IGraphics->RectFill(&trp, 4, 2, w - 7, h - 3);
		IGraphics->SetAPen(&trp, bpen);
		IGraphics->WritePixel(&trp, 4, 2);
		IGraphics->WritePixel(&trp, 4, h - 3);
		IGraphics->WritePixel(&trp, w - 7, 2);
		IGraphics->WritePixel(&trp, w - 7, h - 3);
	}

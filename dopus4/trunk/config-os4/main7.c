...
{
	NULL, 0, 0, 0, 0, GFLG_GADGHNONE, 0, GTYP_BOOLGADGET, NULL, NULL, NULL, 0, NULL, 0, NULL
};
...
void busy()
{
	if(!busyflag)
	{
		...
		IIntuition->AddGList(Window, &busygadget, 0, 1, NULL);
		oldidcmp = Window->IDCMPFlags;
		IIntuition->ModifyIDCMP(Window, IDCMP_CLOSEWINDOW);
...
void unbusy()
{
	if(busyflag)
	{
		IIntuition->RemoveGList(Window, &busygadget, 1);
		IIntuition->ModifyIDCMP(Window, oldidcmp);
		Window->Flags &= ~WFLG_RMBTRAP;
...
doload(int type, int def)
{
	...
	if(def)
	{
		lchanged = changed = 0;
		IDOpus->SetBusyPointer(Window);
		if((a = IDOpus->ReadConfig(loadnamebuf, newcstuff)) != 1)
		{
			IIntuition->ClearPointer(Window);
		...
		else
		{
			lchanged = 1;
			IIntuition->ClearPointer(Window);
			IDOpus->CheckConfig(newcstuff);
		}
	}
	else
	{
		IDOpus->DefaultConfig(newcstuff);
		lchanged = 1;
	}
	if(lchanged)
	{
		...
		case CFG_MAINMENU:
			...
			break;
		case CFG_OPERATION:
			IExec->CopyMem((char *)&newcstuff->config->copyflags, (char *)&config->copyflags, (offsetof(struct Config, dynamicflags) - offsetof(struct Config, copyflags)) + 1);
			...
			for(a = 0; a < 2; a++)
			{
				...
				IExec->CopyMem((char *)newcstuff->config->displaypos[a], (char *)config->displaypos[a], 16);
				IExec->CopyMem((UWORD *) newcstuff->config->displaylength[a], (UWORD *) config->displaylength[a], 32);
			}
			freecon = 3;
			break;
		case CFG_GADGET:
		...
		case CFG_MENU:
			...
			if(!menulist[0] || !(dolistwindow(cfg_string[STR_SELECT_MENUS_TO_IMPORT], 332, 40, menulist, DLVF_MULTI | DLVF_HIREC, menuarray, NULL)))
			{
				ret = 0;
				break;
			}
			for(a = 0, b = 0; a < 5; a++)
			{
				...
				m2 = (a + 1) * 20;
				for(c = m1; c < m2; c++)
					if(config->menu[c].name && config->menu[c].name[0])
						break;
				if(c == m2)
				{
					while(!menuarray[b])
					...
					if(b > 4)
						break;
					for(c = m1; c < m2; c++)
						freenewfunction(&config->menu[c]);
					IExec->CopyMem((char *)&newcstuff->config->menu[b * 20], (char *)&config->menu[m1], sizeof(struct dopusfunction) * 20);
					for(c = m1; c < m2; c++)
					{
		...
		case CFG_DRIVE:
			for(a = 0; a < DRIVECOUNT; a++)
				freestring(config->drive[a].function);
			IExec->CopyMem((char *)newcstuff->config->drive, (char *)config->drive, sizeof(struct dopusfunction) * DRIVECOUNT);
			for(a = 0; a < DRIVECOUNT; a++)
		...
		case CFG_FILETYPE:
			...
			if(!(ftypelist = IDOpus->LAllocRemember(&fkey, (b + 1) * 4, MEMF_CLEAR)) || !(typearray = IDOpus->LAllocRemember(&fkey, b + 1, MEMF_CLEAR)))
				break;
			ftype = newcstuff->firsttype;
			for(a = 0; a < b; a++)
			{
			...
			if(!ftypelist[0] || !(dolistwindow(cfg_string[STR_SELECT_FILETYPES_TO_IMPORT], 332, 72, ftypelist, DLVF_MULTI | DLVF_HIREC, typearray, NULL)))
			{
				ret = 0;
				break;
			}
		...
		case CFG_HOTKEYS:
			...
			while(curhotkey && curhotkey->next)
				curhotkey = curhotkey->next;
			...
			if(!ftypelist[0] || !(dolistwindow(cfg_string[STR_SELECT_HOTKEYS_TO_IMPORT], 332, 72, ftypelist, DLVF_MULTI | DLVF_HIREC, typearray, NULL)))
			{
				ret = 0;
				break;
			}
			...
			break;
		case CFG_SCREEN:
			IExec->CopyMem(newcstuff->config->fontsizes, config->fontsizes, NUMFONTS);
			IExec->CopyMem(newcstuff->config->fontbufs, config->fontbufs, NUMFONTS * 40);
...
void copyconfigonly(struct Config *config1, struct Config *config2)
{
	...
	key = NULL;
	menubackup = (struct newdopusfunction *)getcopy((char *)config2->menu, sizeof(struct dopusfunction) * MENUCOUNT, &key);
	drivebackup = (struct dopusfunction *)getcopy((char *)config2->drive, sizeof(struct dopusfunction) * DRIVECOUNT, &key);
	IExec->CopyMem((char *)config1, (char *)config2, sizeof(struct Config));
	if(menubackup)
		IExec->CopyMem((char *)menubackup, (char *)config2->menu, sizeof(struct newdopusfunction) * MENUCOUNT);
	if(drivebackup)
		IExec->CopyMem((char *)drivebackup, (char *)config2->drive, sizeof(struct dopusfunction) * DRIVECOUNT);
...
dolistwindow(STRPTR title, int w, int h, STRPTR *items, int flags, STRPTR selarray, int *item)
{
	...
	if(!(IDOpus->AddListView(&listlist, 1)))
	{
	...
	IDOpus->AddGadgets(wind, listokaygad, gadtxt, count, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	busy();
		...
		while(IMsg = (struct IntuiMessage *)IExec->GetMsg(wind->UserPort))
		{
			if((view = IDOpus->ListViewIDCMP(&listlist, IMsg)) == (struct DOpusListView *)-1)
			{
				...
				if(class == IDCMP_VANILLAKEY)
				{
					...
					if(a != -1)
					{
						IDOpus->SelectGadget(wind, &listokaygad[a]);
				...
				if(class == IDCMP_GADGETUP)
				{
					...
					{
						unbusy();
						IDOpus->RemoveListView(&listlist, 1);
						IIntuition->CloseWindow(wind);
						return (gadgetid);
					}
					else if(gadgetid == 2 && flags & DLVF_MULTI)
					{
						for(a = 0; a < listlist.count; a++)
							selarray[a] = all;
						IDOpus->RefreshListView(&listlist, 1);
						all = 1 - all;
					}
				}
			}
...
void copytoclip(struct dopusfunction *func, STRPTR *funclist, STRPTR functype, STRPTR flagsel)
{
...
pasteclip(struct dopusfunction *func, STRPTR *funclist, STRPTR functype, STRPTR *displist, STRPTR flagsel)
{
	...
	if(!clipcount)
		return (0);
	if(!(cliplist = IDOpus->LAllocRemember(&key, (clipcount + 1) * 4, MEMF_CLEAR)))
		return (0);
	busy();
	clip = firstclip;
	...
	cliplist[a] = NULL;
	if(dolistwindow(cfg_string[STR_SELECT_FUNCTION_TO_PASTE], 212, 40, cliplist, DLVF_LEAVE | DLVF_HIREC, NULL, &a))
	{
		clip = firstclip;
		for(b = 0; b < a; b++)
			if(!(clip = clip->next))
...
void makefilebuffer(STRPTR buf)
{
	char *ptr;

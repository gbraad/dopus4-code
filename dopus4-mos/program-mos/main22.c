...
int dofilefunction(int function, int flags, STRPTR sourcedir, STRPTR destdir, int act, int inact, int rexx)
{
	...
	int globflag = globflag, noremove, doicons = 0, total, value = 0, progtype = 0, blocksize = 0, retval = 0;
	QUAD byte, bb;
//	ULONG class;
//	USHORT code;
	...
	if(!(database = LAllocRemember(&funckey, 3000, MEMF_CLEAR)))
		return (0);
	sourcename = database;
	...
	case FUNC_ANSIREAD:
	case FUNC_SMARTREAD:
		if(!globflag && !(viewdata = LAllocRemember(&funckey, sizeof(struct ViewData), MEMF_CLEAR)))
			goto endfunction;
	case FUNC_LOOPPLAY:
	case FUNC_SHOW:
		total = -1;
...
//        case FUNC_CLONE:
	...
	case FUNC_HUNT:
		...
		ParsePattern(str_hunt_name, str_hunt_name_parsed, 160);
	...
	while(file)
	{
		...
		if((!file->selected && !globflag) || ((file->type <= ENTRY_FILE) && str_filter_parsed[0] && (MatchPatternNoCase(str_filter_parsed, file->name) == FALSE)))
		{
			file = nextfile;
			continue;
		}
		...
		if(!firstset && !lastfile && !namebuf[0])
			...
			strcpy(namebuf, FilePart(newiconname));
		okayflag = a = 0;
		show = -1;
		...
		case FUNC_DELETE:
			if(file->type >= ENTRY_DIRECTORY)
			{
				...
				{
					if((a = IoErr()) == ERROR_DELETE_PROTECTED && config->deleteflags & DELETE_SET)
					{
						if(!flag)
						{
							flag = 1;
		...
		case FUNC_RENAME:
			...
			{
				if(lastfile)
					StrCombine(destname, sourcedir, newiconname, 256);
				else
				{
				...
				else
				{
					if(file->type >= ENTRY_DIRECTORY)
					{
						StrCombine(buf, sourcedir, file->name, 256);
						TackOn(buf, NULL, 256);
						StrCombine(buf1, sourcedir, namebuf, 256);
						TackOn(buf1, NULL, 256);
		...
		case FUNC_MOVE:
			...
			if(!(Rename(sourcename, destname)))
			{
				if((exist < 0 || file->type <= ENTRY_FILE) && (a = IoErr()) != ERROR_RENAME_ACROSS_DEVICES)
				{
					doerror(a);
...
/*					else
		...
		case FUNC_COMMENT:
			...
			if(!noremove && file->comment != dos_copy_comment)
			{
				...
				else
				{
					...
					if((file->comment = AllocPooled(dir_memory_pool, strlen(buf2) + 1)))
		...
		case FUNC_PROTECT:
			...
			FOREVER
			{
				if(SetProtection(sourcename, temp))
					break;
				doerror((a = IoErr()));

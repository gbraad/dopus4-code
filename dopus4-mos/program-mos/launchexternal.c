...
STATIC int start_external(struct dopus_func_start *func)
{
	struct Process *proc;
	LONG pri;
	int arg;
	...
	if(!(func->replyport = CreateMsgPort()))
		return (0);
	if(!(func->startup.wbstartup.sm_ArgList = LAllocRemember(&func->key, sizeof(struct WBArg) * (func->argcount + 1), MEMF_CLEAR)))
		return (0);
	...
	if(!func->segment)
	{
		...
		Permit();
		if(!func->segment && !(func->segment = (BPTR) LoadSeg(func->segname)))
			return (0);
	}
	...
	NewGetTaskAttrs(NULL, &pri, sizeof(pri), TASKINFOTYPE_PRI, TAG_DONE);

	proc = CreateNewProcTags(
		NP_Name, (Tag) func->procname,
		NP_Priority, pri,
		NP_Seglist, func->startup.wbstartup.sm_Segment,
		NP_PPCStackSize, func->stack * 2,
		NP_FreeSeglist, func->resseg ? FALSE : TRUE,
	...
	func->startup.wbstartup.sm_Process = &proc->pr_MsgPort;
...
STATIC int close_external(struct dopus_func_start *func, int wait)
{
...
void doconfig()
{
	...
	conport->mp_Node.ln_Name = replyname;
...
void dopus_print(int rexx, struct DOpusArgsList *arglist, int printdir, char *port, struct ViewData *vdata)
{
	...
	{
		NewRawDoFmt("!%d", NULL, arglistbuf, arglist);
	}
	...
	for(;;)
	{
		...
		if(vdata)
		{
			...
			while((dmsg = (struct DOpusMessage *)GetMsg(vdata->view_port)))
			{
				...
				case DOPUSMSG_GETVIS:
					CopyMem((char *)&vdata->view_vis_info, (char *)dmsg->data, sizeof(struct VisInfo));
					break;
		...
		if(GetMsg(print_func.replyport))
			break;
	}
...
int dopus_iconinfo(char *filename)
{
	...
	BPTR plock, flock = Lock(filename, ACCESS_READ);
	char buffer[108] = { 0, };
	int a;
	...
	if(a > 5 && strcmp(&buffer[a - 5], ".info") == 0)
	{
		...
		if(infoscreen && plock)
			WBInfo(plock, buffer, infoscreen);
		UnLock(plock);
	}
	if(!MainScreen)
	{
		UnlockPubScreen(NULL, infoscreen);
	}
	UnLock(flock);
	return 0;
}

...
const struct Image uniconifyimage[2] =
{
...
void iconify(int louise, int buttons, int banknum)
{
	...
	else
	{
		...
		{
			if(!(appmenu = AddAppMenuItemA(0, 0, str_arexx_portname, appmsg_port, 0)) || !(appicon = AddAppIconA(0, 0, "Directory Opus", appmsg_port, 0, user_appicon == NULL ? &iconify_appicon : user_appicon, NULL)))
			{
	...
	if(!(icon_type & (ICON_NOWINDOW | ICON_APPICON)))
	{
		...
		if(!(Window = (struct Window *)OpenWindow(&icon_win)))
			goto uniconify;
		...
		if(buttons)
		{
			...
			SetFont(main_rp, winfont);

		SetDrMd(main_rp, JAM2);
		if(icon_gotclock)
		{
			if((iconify_time_port = CreateMsgPort()))
			{
		...
		{
			if(buttons)
				SetWindowTitles(Window, globstring[STR_BUTTON_STRIP], (char *)-1);
			else
				SetWindowTitles(Window, "DirOpus", (char *)-1);
		}
	}
	else if(icon_type & ICON_APPICON)
	{
		Window = OpenWindowTags(&appicon_win, WA_PubScreenName, (Tag) "Workbench", WA_PubScreenFallBack, TRUE, TAG_END);
	}
		...
		if(Window && wmes & 1 << Window->UserPort->mp_SigBit)
		{
			while(getintuimsg())
			{
				...
				case IDCMP_GADGETDOWN:
					...
					FixSliderPot(Window, &iconbutpropgad, (a * bankstep) + (data_gadgetrow_offset / buttonrows), (bankcount * 6) / buttonrows, 1, 1);
					break;
				case IDCMP_GADGETUP:
					if(!buttons)
						break;
...
void remiclock()
{
	if(!(CheckIO(&iconify_timereq.tr_node)))
		AbortIO(&iconify_timereq.tr_node);
	WaitIO(&iconify_timereq.tr_node);
	CloseDevice(&iconify_timereq.tr_node);
	DeleteMsgPort(iconify_time_port);
}
...
void iconstatustext(STRPTR buf, int buttons)
{

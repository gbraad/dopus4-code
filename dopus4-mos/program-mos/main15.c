...
/*			if(a == 1 || a == -1)
	...
	if(a > 5 && strcmp(&name[a - 5], ".info") == 0)
	{
		...
		BPTR plock, flock = Lock(buf, ACCESS_READ);
		char buffer[108] = { 0, };
...
int filesearch(STRPTR name, int *found, int skipall)
{
	char *message;
	int rec, in, suc;
...
void dragcompgad(int gad, int state)
{
	if(main_gadgets[gad].GadgetRender)
	{
		if(state)
		{
			DrawBorder(main_rp, main_gadgets[gad].SelectRender, main_gadgets[gad].LeftEdge, main_gadgets[gad].TopEdge);
		}
		else
		{
			DrawBorder(main_rp, main_gadgets[gad].GadgetRender, main_gadgets[gad].LeftEdge, main_gadgets[gad].TopEdge);
		}
	}
	else
	{

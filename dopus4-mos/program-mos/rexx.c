...
void rexx_dispatch(int allfuncs)
{
	...
	{
		if(!RexxSysBase || (!IsRexxMsg((struct RexxMsg *)msg) && msg->rm_Node.mn_Node.ln_Type != NT_REPLYMSG))
		{
...
//						if((strncmp(commandlist[cmd].name, command, a)) == 0 && (command[a] == 0 || (_isspace(command[a]))))
						if((Strnicmp(commandlist[cmd].name, command, a)) == 0 && (command[a] == 0 || (_isspace(command[a]))))
						{
...
struct RexxMsg *send_rexx_command(STRPTR command, int (*replyfunc) (), struct RexxMsg *msg)
{
	...
	if((RexxMsg = (struct RexxMsg *)CreateRexxMsg(arexx_port, "dopus", str_arexx_portname)) && (RexxMsg->rm_Args[0] = (STRPTR)CreateArgstring(command, (long)strlen(command))))
	{
...
void rexx_copyresult(char *result)
{
	...
	if(result)
		if((new_lrr = AllocVec(strlen(result) + 1, MEMF_ANY)))
		{

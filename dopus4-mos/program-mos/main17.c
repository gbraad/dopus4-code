...
int rexxdisp(struct RexxMsg *msg, struct CommandList *cmd, STRPTR command)
{
	...
	case FUNC_LOADCONFIG:
		if(rexx_argcount > 0 && checknewconfig() && checkwindowquit())
		{
			...
			if((ptr = strstri(FilePart(str_config_basename), ".CFG")))
				*ptr = 0;
	...
	case FUNC_PATTERNMATCH:
		if(rexx_argcount < 2)
			break;
		ParsePatternNoCase(rexx_args[0], buf, 520);
		rexx_return(msg, MatchPatternNoCase(buf, rexx_args[1]));
		return (1);
...
char *dosstatus(int f, STRPTR buf, STRPTR buf1)
{
	if(f)
		strcpy(buf, buf1);
	return (buf);

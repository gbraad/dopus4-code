...
int copyfile(CONST_STRPTR src, CONST_STRPTR dst, int *err, CONST_STRPTR password, int encryptstate)
{
	...
	if(!infile && (!(inhandle = Open(src, MODE_OLDFILE))))
		goto failed;

	if(!(outhandle = Open(dst, MODE_NEWFILE)))
		goto failed;
	...
	while(buffer_size > 0)
	{
		if((buffer = AllocMem(buffer_size, MEMF_ANY)))
			break;
		buffer_size /= 2;
	}
	if(!buffer)
	...
	while(size > 0)
	{
		...
		if(infile)
			length = ReadAsync(infile, buffer, readsize);
		else
			length = Read(inhandle, buffer, readsize);
		...
		if(length > 0)
		{
			if((Write(outhandle, buffer, length)) == -1)
				goto failed;
		}
		size_write += length;
...
struct Directory *findfile(struct DirectoryWindow *dir, CONST_STRPTR name, int *count)
{
...
int delfile(CONST_STRPTR name, STRPTR nam, STRPTR errs, int unprotect, int errcheck)
{
	...
	if(!suc)
	{
		if((err = IoErr()) == ERROR_OBJECT_NOT_FOUND)
			suc = 1;
		else
		{

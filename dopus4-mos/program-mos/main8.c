...
int checkexistreplace(STRPTR sourcename, STRPTR destname, struct DateStamp *date, int allabort, int all)
{
	...
	else if(config->existflags & REPLACE_OLDER)
	{
		...
		if(suc_dfib)
		{
			if(CompareDates(date, &(d_fib.fib_Date)) > 0)
				return (REPLACE_OK);
			doerror(ERROR_OBJECT_EXISTS);
		}
	}
	else if(config->existflags & REPLACE_ASK)
	{
		...
		{
			sprintf(buf, globstring[STR_FILE_EXISTS_REPLACE], FilePart(destname));
		}
		else
		{
		...
		do
		{
			...
			if(a == REPLACE_RENAME)
			{
				...
				strcpy(dname, FilePart(sourcename));
...
void dosizedirwindows(int size)
{
	...
	if(fail == -1 && ox != config->windowdelta && !status_iconified)
	{
		...
		RectFill(main_rp, scrdata_xoffset, horiz_propgad[0].TopEdge - 2, scrdata_xoffset + scrdata_width - 1, path_strgadget[0].TopEdge + path_strgadget[0].Height + 1);
		setupdisplay(0);
		for(x = 0; x < 2; x++)
			RefreshStrGad(&path_strgadget[x], Window);
		okay();
		unbusy();
	}
}
...
void set_reqobject(struct TagItem *object, ULONG tagmatch, ULONG data)
{
	int tag;
...
void FadeRGB32(struct Screen *screen, ULONG *cmap, int count, int dir, int fadetime)
{
	ULONG *tempmap;
	...
	if(!(tempmap = AllocMem(((count * 3) + 2) * sizeof(ULONG), MEMF_CLEAR)) || fadetime < 1 || !(fade_port = CreateMsgPort()) || !(GetDisplayInfoData(NULL, (char *)&dinfo, sizeof(struct DisplayInfo), DTAG_DISP, GetVPModeID(&screen->ViewPort))) || (OpenDevice(TIMERNAME, UNIT_VBLANK, &treq.tr_node, 0)))
	{
		if(tempmap)
		{
	...
	DeleteMsgPort(fade_port);
	if(tempmap)
		FreeMem(tempmap, ((count * 3) + 2) * sizeof(ULONG));
}
...
void FadeRGB4(struct Screen *screen, UWORD *cmap, int count, int dir, int fadetime)
{
	...
	if(fadetime < 1 || !(tempmap = AllocMem(count * sizeof(UWORD), MEMF_CLEAR)) || !(fade_port = CreateMsgPort()) || (OpenDevice(TIMERNAME, UNIT_VBLANK, &treq.tr_node, 0)))
	{
	...
	DeleteMsgPort(fade_port);
	if(tempmap)
		FreeMem(tempmap, count * sizeof(UWORD));
}

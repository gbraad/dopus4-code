...
doload(int type, int def)
{
	...
	if(def)
	{
		...
		if(!(strstri(loadnamebuf, ".CFG")))
			StrConcat(loadnamebuf, ".CFG", 256);
	}
	if(!(newcstuff = AllocMem(sizeof(struct ConfigStuff), MEMF_CLEAR)) || !(newcstuff->config = AllocMem(sizeof(struct Config), MEMF_CLEAR)))
	{
		if(newcstuff)
	...
	if(lchanged)
	{
		...
		case CFG_MAINMENU:
			...
			break;
		case CFG_OPERATION:
			CopyMem((char *)&newcstuff->config->copyflags, (char *)&config->copyflags, (offsetof(struct Config, dynamicflags) - offsetof(struct Config, copyflags)) + 1);
		...
		case CFG_MENU:
			...
			for(a = 0, b = 0; a < 5; a++)
			{
				...
				if(c == m2)
				{
					...
					CopyMem((char *)&newcstuff->config->menu[b * 20], (char *)&config->menu[m1], sizeof(struct dopusfunction) * 20);
					for(c = m1; c < m2; c++)
					{
		...
		case CFG_FILETYPE:
			...
			if(!(ftypelist = LAllocRemember(&fkey, (b + 1) * 4, MEMF_CLEAR)) || !(typearray = LAllocRemember(&fkey, b + 1, MEMF_CLEAR)))
				break;
			ftype = newcstuff->firsttype;
			for(a = 0; a < b; a++)
			{
		...
		case CFG_SCREEN:
			...
			CopyMem((char *)newcstuff->config->scrollborders, (char *)config->scrollborders, sizeof(struct Rectangle) * 2);
			CopyMem((char *)newcstuff->config->new_palette, (char *)config->new_palette, sizeof(ULONG) * 48);
			strcpy(config->pubscreen_name, newcstuff->config->pubscreen_name);
			config->slider_pos = newcstuff->config->slider_pos;
			freecon = 3;
			break;
		case CFG_SYSTEM:
			CopyMem((char *)newcstuff->config->outputcmd, (char *)config->outputcmd, offsetof(struct Config, gadgetrows) - offsetof(struct Config, outputcmd));
			CopyMem((char *)newcstuff->config->startupscript, (char *)config->startupscript, offsetof(struct Config, pad8) - offsetof(struct Config, startupscript));
	...
	if(freecon & 2)
		FreeConfig(newcstuff);
	if(freecon & 1)
		FreeMem(newcstuff->config, sizeof(struct Config));
	FreeMem(newcstuff, sizeof(struct ConfigStuff));
	cstufffix(&cstuff);
	LFreeRemember(&fkey);
	unbusy();
...
void copyconfigonly(struct Config *config1, struct Config *config2)
{
	...
	if(menubackup)
		CopyMem((char *)menubackup, (char *)config2->menu, sizeof(struct newdopusfunction) * MENUCOUNT);
	if(drivebackup)
		CopyMem((char *)drivebackup, (char *)config2->drive, sizeof(struct dopusfunction) * DRIVECOUNT);
	LFreeRemember(&key);
}
...
dolistwindow(STRPTR title, int w, int h, STRPTR *items, int flags, STRPTR selarray, int *item)
{
	...
	AddGadgets(wind, listokaygad, gadtxt, count, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	busy();
...
pasteclip(struct dopusfunction *func, STRPTR *funclist, STRPTR functype, STRPTR *displist, STRPTR flagsel)
{
	...
	if(!clipcount)
		return (0);
	if(!(cliplist = LAllocRemember(&key, (clipcount + 1) * 4, MEMF_CLEAR)))
		return (0);
	busy();
	clip = firstclip;
	for(a = 0; a < clipcount; a++)
	{
		if(!clip)
			break;
		if(!(cliplist[a] = LAllocRemember(&key, 40, MEMF_CLEAR)))
			break;

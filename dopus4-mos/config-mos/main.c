...
int main(int argc, char **argv)
{
	...
	LONG num, a;
	TEXT portname[50], rportname[50], oldconfigname[256];
	...
	DOpusBase = OpenLibrary("dopus.library", LIB_VER);
	...
	if(!(FindSystemFile("DirectoryOpus.CFG", configname, 256, SYSFILE_DATA)))
	{
		...
		{
			stccpy(configname, "PROGDIR:/S/DirectoryOpus.CFG", 256);
		}
		else
		{
			stccpy(configname, "s:DirectoryOpus.CFG", 256);
		}
	}

	if(!(FindSystemFile("DirectoryOpus.CLA", classname, 256, SYSFILE_DATA)))
	{
		classname[0] = 0;
	}
	...
	else
	{
		...
		{
			StrToLong(wbmsg->sm_ArgList[1].wa_Name, &num);
			...
			conport->mp_Node.ln_Name = portname;
			conport->mp_Node.ln_Pri = 20;
			AddPort(conport);
	...
	for(a = 0; a < 13; a++)
	{
		if(!(functypelist[a] = LAllocRemember(&mainkey, 40, MEMF_CLEAR)))
		{
			quit();
		}
	}
	...
	getconfig();
	stccpy(oldconfigname, configname, 256);
...
void getconfig()
{
	...
	if(cmdport)
	{
		...
		typekey = cfg->typekey;
		changed = cfg->changed;
		stccpy(configname, cfg->configname, 256);
		strncat(configname, ".CFG", 256);
		stccpy(loadnamebuf, configname, 256);
		Screen = cfg->Screen;
	}
	else
	{
...
{
	fixcstuff(&cstuff);
	if((ReadConfig(configname, &cstuff)) != 1)
...
void cleanconfigscreen()
{
	...
	RectFill(rp, x_off, y_off, x_bot, y_bot);
...
void showconfigscreen(int scr)
{
	...
	case CFG_GADGET:
		if(gadgads)
		{
			AddGadgets(Window, gadgads, NULL, 84, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
		}
		...
		BltTemplate((UBYTE *)uparrow_data, 0, 2, rp, x_off + 179, y_off + 66, 12, 6);
		UScoreText(rp, cfg_string[STR_RIGHT_MOUSE_BUTTON], x_off + 454, y_off + 75, -1);
		BltTemplate((UBYTE *)downarrow_data, 0, 2, rp, x_off + 437, y_off + 70, 12, 6);

	case CFG_MENU:
		if(scr == CFG_MENU)
		{
			if(menugads)
				AddGList(Window, menugads, -1, MENUCOUNT + 5 - (HIDDENLINES * 5), NULL);
			...
			AddGadgets(Window, &menuslidergads[1], NULL, 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 0);
			do3dbox(x_off + 612, y_off + 21, 12, 144 - HIDDENLINES * 8);
			DoArrow(rp, x_off + 612, y_off + 119, 12, 6, screen_pens[config->arrowfg].pen, screen_pens[config->arrowbg].pen, 0);
			DoArrow(rp, x_off + 612, y_off + 127, 12, 6, screen_pens[config->arrowfg].pen, screen_pens[config->arrowbg].pen, 1);
	...
	case CFG_DRIVE:
		...
		{
			if(drivegads)
				AddGList(Window, drivegads, -1, DRIVECOUNT, NULL);
...
void initsidegads(STRPTR *gads, int toggle, int vert)
{
	...
	SetAPen(rp, screen_pens[1].pen);
	AddGadgetBorders(&gadgetkey, gadbuf, gnum, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	AddGadgets(Window, gadbuf, gads, num, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	maingad = gadbuf;
}

void inittickgads(struct ConfigGadget *gads, int flag, int flag2)
{
	LONG num, a, y, gad, last = 0, lasty, dy, xp, yp, fl = 0, b;
	char **namearray;
	...
	if(!(tickgad = LAllocRemember(&tickkey, sizeof(struct Gadget) * num, MEMF_CLEAR)) || !(namearray = LAllocRemember(&tickkey, num * 4, MEMF_CLEAR)))
		return;
	...
	for(a = 0;; a++)
	{
		...
		case CTYPE_TEXT:
			...
			if(gads[a].nameentry > -1)
				UScoreText(rp, cfg_string[gads[a].nameentry], xp, yp, -1);
			break;
		case CTYPE_GAD:
		case CTYPE_RAD:
		case CTYPE_MRAD:
		...
		case CTYPE_REQ:
			...
			tickgad[gad].UserData = (APTR) & gads[a];
			AddGadgetBorders(&tickkey, &tickgad[gad], 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	...
	AddGadgets(Window, tickgad, namearray, num, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	tickcount = num;
}
...
void removetickgads()
{
	...
	LFreeRemember(&tickkey);
	SetAPen(rp, screen_pens[0].pen);
	RectFill(rp, x_off + 122, y_off + 1, x_bot, y_bot);
}
...
int processtickgad(struct ConfigGadget *gads, int flag, int sel, int num)
{
	...
	if(ref)
		RefreshGList(tickgad, Window, NULL, tickcount);
	return (flag);
}
...
char *getcopy(STRPTR object, int size, struct DOpusRemember **key)
{
	...
	if(key)
		newobject = (char *)LAllocRemember(key, size, MEMF_CLEAR);
	else
		newobject = AllocMem(size, MEMF_CLEAR);
	if(newobject)
		CopyMem((char *)object, (char *)newobject, size);
	return (newobject);
}
...
void doglassimage(struct Gadget *gad)
{
	...
	for(a = 0; a < 2; a++)
	{
		BltTemplate((UBYTE *)glass_data[a], 0, 4, rp, gad->LeftEdge + 4, gad->TopEdge + 2, 20, 9);
	...
	if((msg = (struct IntuiMessage *)GetMsg(Window->UserPort)))
	{
		if(msg->Class == IDCMP_RAWKEY)
		{
			...
			else if(msg->Qualifier & IEQUALIFIER_RCOMMAND && Window->MenuStrip)
			{
				if(!(msg->Code & 0x80))
				{
					RawkeyToStr(msg->Code, 0, NULL, buf, 10);
					c = ToUpper(buf[0]);
...
struct TextFont *getfont(STRPTR font, int *size, int flags)
{
	...
	sfont.ta_YSize = *size;
	if((tf = OpenFont(&sfont)) && tf->tf_YSize == sfont.ta_YSize && (!(tf->tf_Flags & FPF_PROPORTIONAL) || flags != FFLAG_NOPROP))
		return (tf);
	if(tf)
		CloseFont(tf);
...
void load_palette(struct Screen *screen, ULONG *palette, int numcols)
{
	int a = 0, b = 0;
	if(!screen)
	{
		...
		{
			SetRGB32(&Window->WScreen->ViewPort, screen_pens[a].pen, palette[b], palette[b + 1], palette[b + 2]);
			b++; b++; b++;
		}
	}
	else
...
void get_palette(struct Screen *screen, ULONG *palette, int numcols)
{
	int a, b;
	if(!screen)
	{
		...
		{
			GetRGB32(Window->WScreen->ViewPort.ColorMap, screen_pens[a].pen, 1, &palette[b]);
		}
	}
	else
	{
...
//					if((screen_pens[pen].pen = ObtainPen(cm, -1, screen_pens[pen].red, screen_pens[pen].green, screen_pens[pen].blue, 0/*PEN_EXCLUSIVE*/)) == (UBYTE)-1)
					if((screen_pens[pen].pen = ObtainBestPenA(cm, screen_pens[pen].red, screen_pens[pen].green, screen_pens[pen].blue, NULL)) == (UBYTE)-1)
					{
						break;
					}
		...
		if(!onworkbench)
		{
			scr_taglist[1].ti_Data = BestModeID(BIDTAG_DesiredWidth, configscr.Width, BIDTAG_DesiredHeight, configscr.Height, BIDTAG_Depth, configscr.Depth, wbscreen ? BIDTAG_MonitorID : TAG_IGNORE, wbscreen ? GetVPModeID(&wbscreen->ViewPort) & MONITOR_ID_MASK : 0, TAG_END);
		...
		if(!(Window = OpenWindow(&configwin)))
		{
			quit();
		}
		...
		if(!Screen)
		{
			SetWindowTitles(Window, (char *)-1, configscr.DefaultTitle);
		}

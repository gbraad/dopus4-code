...
int editfunction(struct dopusfunction *func, int type, APTR data)
{
	...
	AddGadgetBorders(&key, &editfuncgadgets[2], 4, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	AddGadgetBorders(&key, &editfuncgadgets[9], 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	AddGadgetBorders(&key, &editfuncgadgets[10], 3, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	AddGadgetBorders(&key, &editfuncgadgets[14], 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	...
	if(type == CFG_HOTKEYS)
	{
		...
		AddGadgets(Window, editfuncgadgets, editfuncgads2, 13, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	}
	...
	else if(type == CFG_FILETYPE)
	{
		...
		AddGadgets(Window, &editfuncgadgets[2], &editfuncgads2[2], 13, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	}
...
char *compilefunclist(STRPTR *funclist, STRPTR functype, struct DOpusRemember **key)
{
	...
	if(key)
		buf = LAllocRemember(key, size, MEMF_CLEAR);
	else
		buf = AllocMem(size, MEMF_CLEAR);
	if(!buf)
		return (NULL);
	buf2[1] = 0;
	for(a = 0; a < num; a++)
...
void drawcompbox(struct RastPort *r, int x1, int y1, int w, int h)
{
	...
	Move(r, x1 + 2, y1 + 1);
	Draw(r, x1 + w - 3, y1 + 1);
	Draw(r, x1 + w - 3, y1 + h - 2);
	Draw(r, x1 + 2, y1 + h - 2);
	Draw(r, x1 + 2, y1 + 2);
	Draw(r, x1 + 3, y1 + 2);
	Draw(r, x1 + 3, y1 + h - 3);
	Move(r, x1 + w - 4, y1 + 2);
	Draw(r, x1 + w - 4, y1 + h - 3);
	SetDrMd(r, JAM2);
}
...
void editgadsoff()
{
	...
	for(a = 9; a < 13; a++)
		DisableGadget(&editfuncgadgets[a], rp, (a == 10 || a == 12) ? 3 : ((a == 9) ? 4 : 0), (a == 11) ? 0 : 2);
}
...
int funcrequester(int type, STRPTR buf, STRPTR title)
{
	...
	case FREQ_FILECLASS:
		...
		AddGadgets(wind, &cmdcancelgad, &editfuncgads[4], 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	...
	default:
		...
		case FT_AREXX:
			filebuf[0] = 0;
			if(CheckExist(defdir[type], NULL))
...
void setupeditdisplay(struct dopusfunction *func, int type, APTR data, int curtype, STRPTR *displist, STRPTR *funclist, STRPTR functype, STRPTR flagsel, int add)
{
	...
	RefreshGList(&editfuncgadgets[6], Window, NULL, 3);
	if(type == CFG_FILETYPE)
		RefreshGList(&editfuncgadgets[13], Window, NULL, 1);
}
...
void add_appobject(int type)
{
	if(WorkbenchBase && appport)
	{
		if(Screen && type)
		{
			appobject = (void *)AddAppIconA(MY_APPOBJECT, 0, cfg_string[STR_DROP_A_TOOL_HERE], appport, 0, dropboxicon, NULL);
		}
		else if(!Screen)
		{
			appobject = (void *)AddAppWindowA(MY_APPOBJECT, 0, Window, appport, NULL);
		}
	}
}

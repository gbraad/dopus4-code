...
void makehotkeylist()
{
	...
	if(count && (hotkeylist = LAllocRemember(&hkey, (count + 1) * 4, MEMF_CLEAR)) && (buf = LAllocRemember(&hkey, count * 80, MEMF_CLEAR)))
	{
		hotkey = firsthotkey;
		for(a = 0; a < count; a++)
		{
			hotkeylist[a] = &buf[a * 80];
			RawkeyToStr(hotkey->code, hotkey->qualifier, str, NULL, 39);
			str[39] = 0;
...
void doradiobuttons()
{
	...
	buttonoffimage = GetButtonImage(15, 9, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, screen_pens[0].pen, screen_pens[0].pen, &buttonkey);

	buttononimage = GetButtonImage(15, 9, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, screen_pens[3].pen, screen_pens[0].pen, &buttonkey);

	if(checkoffimage = GetCheckImage(screen_pens[1].pen, screen_pens[0].pen, 0, &buttonkey))
		CopyMem((char *)checkoffimage, (char *)&copy_checkoffimage, sizeof(struct Image));
	else
		copy_checkoffimage.ImageData = NULL;

	if(checkonimage = GetCheckImage(screen_pens[1].pen, screen_pens[0].pen, 1, &buttonkey))
	...
	CreateGadgetBorders(&buttonkey, 104, 12, &gadget_dog_sel_border, &gadget_dog_unsel_border, 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);

	CreateGadgetBorders(&buttonkey, 104, 12, &gadget_sel_border, &gadget_unsel_border, 0, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
...
void select_gadget(struct Gadget *gad, int state)
{
	...
	{
		if(state)
			DrawBorder(rp, gad->SelectRender, gad->LeftEdge, gad->TopEdge);
		else
			DrawBorder(rp, gad->GadgetRender, gad->LeftEdge, gad->TopEdge);
	}

...
int initscreenmodes()
{
	int count, minw, minh, depth, oldcount = 0;
	...
	{
		if(!ModeNotAvailable(modeid) && (handle = FindDisplayInfo(modeid)) && (GetDisplayInfoData(handle, namebuf, 180, DTAG_NAME, 0)) && (GetDisplayInfoData(handle, buf, 256, DTAG_DIMS, 0)))
		{
			minw = dimension->Nominal.MaxX + 1;
			minh = dimension->Nominal.MaxY + 1;

			if((dimension->Nominal.MaxX >= 639) && (dimension->Nominal.MaxY >= 479))
				count += addscreenmode(((struct NameInfo *)namebuf)->Name, minw, minh, minw, minh, minw, minh, dimension->MaxDepth, modeid);
		}
	}
...
int addscreenmode(STRPTR name, UWORD minw, UWORD minh, UWORD maxw, UWORD maxh, UWORD defw, UWORD defh, UWORD depth, ULONG mode)
{
	...
	while(screenmode)
	{
		if(LStrCmpI(screenmode->name, name) == 0)
			return (0);
		if(!screenmode->next)
			break;
...
void fixmodegads(struct ScreenMode *mode)
{
	...
	if(mode)
	{
		...
		{
			FixSliderBody(Window, &screenmodegads[SCREENMODE_SLIDER - 300], mode->maxdepth - 1, 1, 0);
		}
	}
	...
	for(gap = count / 2; gap > 0; gap /= 2)
		for(i = gap; i < count; i++)
			for(j = i - gap; j >= 0; j -= gap)
			{
				...
				if(!mode1 || !mode2 || LStrCmpI(mode1->name, mode2->name) <= 0)
					break;

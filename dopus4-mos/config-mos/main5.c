...
char **makefiletypelist(struct DOpusRemember **key)
{
	...
	if(!(list = LAllocRemember(key, (count + 1) * 4, MEMF_CLEAR)))
		return (NULL);
	type = firsttype;
	for(a = 0; a < count; a++)
	{
...
void readfileclasses()
{
	...
	if((CheckExist(classname, &size)) >= 0 || !(classbuf = AllocMem(size, MEMF_CLEAR)))
		return;
...
int importfileclasses()
{
	...
	{
		sprintf(buf2, cfg_string[STR_OPEN_FAILED], IoErr());
		if(!(request(buf2)))
			return (0);
	}
	...
	if((CheckExist(buf, &size)) >= 0 || !(classbuf = LAllocRemember(&key, size + 2, MEMF_CLEAR)))
	{
		Close(in);
		return (0);
	}
	...
	if((classlist = LAllocRemember(&key, (num + 1) * 4, MEMF_CLEAR)) && (classtypeid = LAllocRemember(&key, (num + 1) * 4, MEMF_CLEAR)) && (classrecog = LAllocRemember(&key, (num + 1) * 4, MEMF_CLEAR)) && (classarray = LAllocRemember(&key, num + 1, MEMF_CLEAR)))
	{
		pos = 0;
		a = 0;
		for(;;)
...
int savefileclasses()
{
	...
	{
		sprintf(buf, cfg_string[STR_SAVE_FAILED], IoErr());
		if(!(request(buf)))
			return (0);
	}
	fclass = firstclass;
	while(fclass)
	{
		if(fclass->type[0])
		{
			if((Write(out, fclass->type, (a = strlen(fclass->type)))) < a)
				break;
			if(fclass->typeid[0])
			{
				Write(out, &g, 1);
				if((Write(out, fclass->typeid, (a = strlen(fclass->typeid)))) < a)
					break;
			}
		}
		Write(out, &f, 1);
...
int addfileclass(STRPTR type, STRPTR typeid, STRPTR recog)
{
	...
	while(fclass)
	{
		if(LStrCmpI(fclass->type, type) >= 0)
			break;
		if(!fclass->next)
			last = fclass;
		fclass = fclass->next;
	}
	if(!(newclass = AllocMem(sizeof(struct fileclass), MEMF_CLEAR)))
		return (0);
	if(fclass)
	{
		if(fclass->last)
...
char **makeclasslist(struct DOpusRemember **key)
{
	...
	for(a = 0; a < count; a++)
	{
		if(list[a] = LAllocRemember(key, 40, MEMF_CLEAR))
...
int editfileclass(struct fileclass *fclass, int new)
{
	...
	AddGadgetBorders(&key, &editclassgadgets[2], 4, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	AddGadgetBorders(&key, &editclassgadgets[6], 3, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	AddGadgetBorders(&key, &editclassgadgets[10], 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
	AddGadgetBorders(&key, &editclassgadgets[12], 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);

	SetAPen(rp, screen_pens[1].pen);
	AddGadgets(Window, editclassgadgets, editclassgads, 14, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	doglassimage(&editclassgadgets[6]);
	doglassimage(&editclassgadgets[8]);
	Do3DBox(rp, x_off + 37, y_off + 140, 116, 12, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
			...
			if((view = ListViewIDCMP(&editclasslist, IMsg)) == (struct DOpusListView *)-1)
			{
				...
				case IDCMP_MOUSEBUTTONS:
					if(code != SELECTDOWN || mx < x_off + 11 || mx > x_off + 505 || my < y_off + 21 || my > x_off + 68)
						break;
					if(oldsec && DoubleClick(oldsec, oldmic, sec, mic))
						off = 1;
					else
						off = 0;
					oldsec = sec;
	...
	for(a = 0; a < MAXFUNCS; a++)
	{
		...
		*ptr = 0;
		...
		if(classopvals[classtype[a] - 1] < FTYC_COMMANDOK)
			StrConcat(displist[a], classlist[a], 75);
	}
	for(; a < MAXFUNCS; a++)
		displist[a][0] = 0;
}
...
void seteditclassgads(int on)
{
	...
	{
		for(a = 3; a < 6; a++)
			EnableGadget(&editclassgadgets[a], rp, 4, 2);
	...
	{
		for(a = 3; a < 6; a++)
			DisableGadget(&editclassgadgets[a], rp, 4, 2);
...
void makeclassrecog(struct fileclass *class, STRPTR *classlist, STRPTR classtype)
{
	...
	if(!size)
		return;
	if(!(buf = AllocMem(size + 2, MEMF_CLEAR)))
		return;
	buf2[1] = 0;
	for(a = 0; a < num; a++)
	{
...
void showclassop(int op)
{
	...
	RectFill(rp, x_off + 37, y_off + 140, x_off + 152, y_off + 151);
	SetAPen(rp, screen_pens[1].pen);
	if(op < 1)
		ptr = "----";
	else
...
void draw_file_view()
{
	...
	SetAPen(rp, screen_pens[0].pen);
	RectFill(rp, y_off + 7, y_off + 21, x_off + 511, y_off + 68);
	do3dbox(x_off + 7, y_off + 21, 505, 48);
	Do3DBox(rp, x_off + 534, y_off + 31, 91, 12, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	Do3DBox(rp, x_off + 534, y_off + 57, 91, 12, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	SetAPen(rp, screen_pens[1].pen);
	UScoreText(rp, cfg_string[STR_POSITION], x_off + 535, y_off + 27, -1);
	UScoreText(rp, cfg_string[STR_OFFSET], x_off + 535, y_off + 53, -1);
	for(a = 0; a < 2; a++)
		DoArrow(rp, editclassgadgets[12 + a].LeftEdge + 2, editclassgadgets[12 + a].TopEdge + 1, 12, 6, screen_pens[1].pen, screen_pens[0].pen, a);
...
void load_file_view()
{
	...
	free_file_view();
	busy();
	if((CheckExist(edit_pathbuf, &fileview_size)) < 0 && (file = Open(edit_pathbuf, MODE_OLDFILE)))
	{
...
void file_view_text(STRPTR txt, int line)
{
	...
	if(txt)
		Text(rp, txt, (len = strlen(txt)));
	else
		len = 0;
	if(len < 62)
		Text(rp, spacestring, 62 - len);
}
		...
		if(line >= top && line <= bottom)
		{
			...
			else
			{
				...
				if(px > -1 && px != ox)
				{
					...
					RectFill(rp, x_off + 9 + (10 + px) * char_w, y_off + 21 + (line * 8), x_off + 8 + (12 + px) * char_w, y_off + 28 + (line * 8));
					RectFill(rp, x_off + 9 + (10 + apx) * char_w, y_off + 21 + (line * 8), x_off + 8 + (11 + apx) * char_w, y_off + 28 + (line * 8));
					SetDrMd(rp, JAM2);
				}
				if(ox > -1)
				{
...
void show_view_number(int num, int pos)
{
	...
	{
		SetAPen(rp, screen_pens[0].pen);
		RectFill(rp, x_off + 534, y_off + 31 + (26 * pos), x_off + 624, y_off + 42 + (26 * pos));
		SetAPen(rp, screen_pens[1].pen);
	}
	else
	{

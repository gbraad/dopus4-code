...
void readhelp()
{
	...
	if(!config->language[0] || !(FindSystemFile(helpname, helpfilename, 256, SYSFILE_DATA)))
		FindSystemFile("ConfigOpus.HLP", helpfilename, 256, SYSFILE_DATA);

	if(CheckExist(helpfilename, &helpsize) >= 0)
		return;
...
void makehelpname(STRPTR path)
{
	...
	if(path == (STRPTR)-1)
	{
		...
		TackOn(currenthelpname, path, 256);
}
...
void doconfighelp()
{
	...
	for(a = 0; a < helpsize; a++)
	{
		...
		{
			if(LStrCmpI(&helpbuffer[a], currenthelpname) == 0)
				test = -1;
			else
				++test;
		}
...
void dohelpmsg(STRPTR text)
{
	...
	size = strlen(text);
	if(!(buf = LAllocRemember(&key, size + 1, MEMF_CLEAR)))
		return;
	...
	if((wind = openwindow(&requestwin)))
	{
		...
		if(AddListView(&helplist, 1))
		{
			...
			AddGadgets(wind, &helpcancelgad, editfuncgads, 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
			for(;;)
			{
...
void load_clips()
{
	...
	if(!(file = Open("t:configopus.clip", MODE_OLDFILE)))
		return;
	...
	for(;;)
	{
		if((Read(file, (char *)&clip, sizeof(struct Clip))) < sizeof(struct Clip))
			break;
		if(clip.func.function && (funcbuf = LAllocRemember(&clipkey, (int)clip.func.function, 0)))
		{
...
void save_clips()
{
	...
	if(!(file = Open("t:configopus.clip", MODE_NEWFILE)))
		return;
	...
	while(clip)
	{
		...
		if((Write(file, (char *)clip, sizeof(struct Clip))) < sizeof(struct Clip))
			break;
		if(function && ((Write(file, (char *)function, (int)clip->func.function)) < (int)clip->func.function))
			break;
		clip = clip->next;
	}
...
int readfile(STRPTR name, STRPTR *buf, int *size)
{
	...
	if(CheckExist(name, size) >= 0 || !(in = Open(name, MODE_OLDFILE)))
		return (-1);

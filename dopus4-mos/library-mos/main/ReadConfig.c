...
int _DOpus_ReadConfig(STRPTR name, struct ConfigStuff *cstuff)
{
	...
	FreeConfig(cstuff);
	if(CheckExist(name, &size) >= 0)
		return (ERROR_OBJECT_NOT_FOUND);
	size -= sizeof(struct Config);
	if(!(in = Open(name, MODE_OLDFILE)))
		return (IoErr());

	if((Read(in, (STRPTR)&ver, 2)) < 2 || (Read(in, (STRPTR)&mag, 2)) < 2 || mag != CONFIG_MAGIC || ver == 0xde)
	{
	...
	if((Read(in, (STRPTR)config, sizeof(struct Config))) < sizeof(struct Config))
	{
		Close(in);
		return (IoErr());
	}
	...
	if(config->version < CONFIG_LESS_DODRIVES)
	{
		...
		for(a = OLDDRIVECOUNT; a < DRIVECOUNT; a++)
			tempfunc[a].fpen = 3;
		CopyMem((STRPTR)tempfunc, (STRPTR)driveptr, sizeof(struct dopusfunction) * DRIVECOUNT);
	...
	if(config->version <= CONFIG_NEW_FUNCTIONS)
	{
		...
		{
			strcpy(buf2, (STRPTR)&config->menu[a]);
	...
	key = NULL;
	if(!(cbuf = (STRPTR)LAllocRemember(&key, size, MEMF_CLEAR)) || !(buf = (STRPTR)LAllocRemember(&key, 4096, MEMF_CLEAR)) || !(tbuf = (STRPTR)LAllocRemember(&key, 256, MEMF_CLEAR)))
	{
		Close(in);
		LFreeRemember(&key);
		return (ERROR_NO_FREE_STORE);
	...
	for(a = 0; a < MENUCOUNT; a++)
	{
		...
		if(buf[0])
			AssignMenu(cstuff, a, (STRPTR)-1, buf);
		if(pos == -1)
			goto endthis;
	}
	if(config->version > CONFIG_NEW_FUNCTIONS)
	{
		for(a = 0; a < MENUCOUNT; a++)
		{
			pos = readline(cbuf, pos, buf, size);
			if(buf[0])
				AssignMenu(cstuff, a, buf, (STRPTR)-1);
			if(pos == -1)
				goto endthis;
		}
	}
	...
	for(;;)
	{
		...
		if(cbuf[pos] == 5 && cbuf[pos + 1] == 'H')
		{
			...
			{
				CopyMem((STRPTR)&cbuf[pos], (STRPTR)hotkey, sizeof(struct dopushotkey));
		...
		else if(cbuf[pos] == 6 && cbuf[pos + 1] == 'F')
		{
			...
			if((pos + sizeof(struct wr_dopusfiletype)) >= size)
				break;
			if((newtype = (struct dopusfiletype *) LAllocRemember(&cstuff->typekey, sizeof(struct dopusfiletype), MEMF_CLEAR)))
			{
				CopyMem((STRPTR)&cbuf[pos], (STRPTR)newtype, sizeof(struct wr_dopusfiletype));
				...
				if((newtype->recognition = LAllocRemember(&cstuff->typekey, strlen(buf) + 1, 0)))
					strcpy((STRPTR)newtype->recognition, buf);
				for(a = 0; a < FILETYPE_FUNCNUM; a++)
				{
					if((pos = readline(cbuf, pos, buf, size)) == -1)
						break;
		...
		else if(cbuf[pos] == 1 && cbuf[pos + 1] == 'F')
		{
			...
			if((newtype = (struct dopusfiletype *)LAllocRemember(&cstuff->typekey, sizeof(struct dopusfiletype), MEMF_CLEAR)))
			{
				CopyMem((STRPTR)&cbuf[pos], (STRPTR)&otype, sizeof(struct olddopusfiletype));
				...
				buf[0] = 0;
				if(otype.filepat[0])
					NewRawDoFmt("%lc%s%lc", NULL, buf, FTYC_MATCHNAME, otype.filepat, ((otype.recogchars[0]) ? ((otype.and) ? FTYC_AND : FTYC_OR) : FTYC_ENDSECTION));
					...
					for(a = 0; a < 100; a++)
					{
						if(!otype.recogchars[a] || otype.recogchars[a] == ',')
						{
							...
							if(mv == 0)
							{
								if((b = Atoh(buf2, 0)) > 0)
								{
									NewRawDoFmt("%lc%ld%lc", NULL, buf3, FTYC_MOVETO, b, FTYC_ENDSECTION);
							...
							{
								NewRawDoFmt("%lc$%s%lc", NULL, buf3, FTYC_MATCH, buf2, FTYC_AND);
								StrConcat(buf, buf3, 4096);
								mv = 0;
							}
		...
		else if((cbuf[pos] == 2 || cbuf[pos] == 3) && cbuf[pos + 1] == 'G')
		{
			...
			if((pos + b) >= size)
				break;
			++bk;
			if(!(temp = AllocMem(sizeof(struct dopusgadgetbanks), MEMF_CLEAR)))
				goto endthis;
			if(!cstuff->firstbank)
			...
			if(a)
			{
				CopyMem((STRPTR)&cbuf[pos], (STRPTR)bank->gadgets, sizeof(struct newdopusfunction) * GADCOUNT);
			...
			else
			{
				...
				{
					CopyMem((STRPTR)&cbuf[pos], (STRPTR)&bank->gadgets[a], 10);
					CopyMem((STRPTR)&cbuf[pos + 10], (STRPTR)&bank->gadgets[a].which, 16);
			...
			for(gad = 0; gad < GADCOUNT; gad++)
			{
				...
				{
					pos = readline(cbuf, pos, buf, size);
					AssignGadget(cstuff, bk, gad, (STRPTR)-1, buf);
				}
			}
			if(config->version > CONFIG_NEW_FUNCTIONS)
			{
				for(gad = 0; gad < GADCOUNT; gad++)
				{
					...
					{
						pos = readline(cbuf, pos, buf, size);
						AssignGadget(cstuff, bk, gad, buf, (STRPTR)-1);
					}
				}
			}
			else
			{
				for(gad = 0; gad < GADCOUNT; gad++)
				{
					strcpy(buf, (STRPTR)&bank->gadgets[gad]);
	...
	if(config->version < CONFIG_USERAW0_HOTKEY)
	{
		...
		strcpy(config->autodirs[1], config->autodirs[0] + 30);
	}

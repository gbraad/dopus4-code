...
char *getstringcopy(char *str)
{
	...
	if(str && (newstr = AllocVecTaskPooled(strlen(str) + 1)))
		strcpy(newstr, str);
	return (newstr);
}
...
int writestring(int file, char *string)
{
	...
	if(string)
	{
		if((Write(file, string, (b = (strlen(string) + 1)))) < b)
			return (0);
	}
	else
		Write(file, &nl, 1);
...
void AssignDrive(struct ConfigStuff *cstuff, int drv, char *name, char *path)
{
	...
	if(name != (char *)-1)
	{
		if(!name)
			...
			stccpy(config->drive[drv].name, name, 16);
	}
	if(path != (char *)-1)
	{
...
void linkinnewfiletype(struct ConfigStuff *cstuff, struct dopusfiletype *temp)
{
	...
	else
	{
		while (pos->next)
		{
			if(Stricmp(pos->next->type, "Default") == 0)
			{
				temp->next = pos->next;
				break;
			}
...
void ShowRMBGadName(struct RastPort *rp, struct RMBGadget *gad, int a)
{
	...
	if((text=&gad->txt[a]))
	{
		...
		if(text->text)
		{
			...
			buf[len] = 0;
			UScoreText(rp, buf, gad->x + text->x + ((text->w - l) / 2), gad->y + text->y + ((text->h - rp->Font->tf_YSize) / 2) + rp->Font->tf_Baseline, uspos);
		}
		SetDrMd(rp, old);
	}
}
...
void HighlightRMBGad(struct RastPort *rp, struct RMBGadget *gad, int state)
{
	if(gad->flags & RGF_ALTBORD)
		DrawBorder(rp,gad->high_bord[state],gad->x,gad->y);
	else
	{
		int old;
...
struct Image *get_image_data(struct DOpusRemember **key, int width, int height, int depth, struct BitMap *bm, struct RastPort *rp)
{
	...
	if(!(image = LAllocRemember(key, sizeof(struct Image), MEMF_CLEAR)) || !(data = LAllocRemember(key, words * 2 * height * depth, MEMF_CHIP|MEMF_CLEAR)))
		return(NULL);
...
void DisplayView(struct DOpusListView *view)
{
	...
	for(a = start; a != end; a += step)
	{
		if(a >= top && a <= bot)
		{
			...
			{
				if(dir < 0)
					ClipBlit(rp, view->xo, view->yo, rp, view->xo, view->yo + view->fh, w, (view->lines - 1) * view->fh, 0xc0);
				else
					ClipBlit(rp, view->xo, view->yo + view->fh, rp, view->xo, view->yo, w, (view->lines - 1) * view->fh, 0xc0);
				if(view->flags & DLVF_CHECK)
					ScrollRaster(rp, 0, dir, view->x, view->yo, view->xo - 1, view->my);
			}
			...
			if(a < view->count)
			{
				if((view->flags & DLVF_MULTI && view->selected[a] & LVARRAY_SELECTED) || (view->flags & DLVF_LEAVE && view->itemselected == a))
				{
					...
					{
						if(view->flags & DLVF_CHECK)
							DrawCheckMark(rp, view->xo - 18, y + view->ty - view->fb, 1);
						else
						{
							SetDrMd(rp, COMPLEMENT);
							if(dir < 0)

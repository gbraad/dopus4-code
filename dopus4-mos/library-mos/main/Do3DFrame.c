...
void _DOpus_Do3DFrame(struct RastPort *rp, int x, int y, int w, int h, char *title, int hi, int lo)
{
	char of,ob;
	int a,l;
	...
	if(title)
	{
		...
		SetAPen(rp, ob);
		RectFill(rp, x - rp->Font->tf_XSize / 2, y + 2 - rp->Font->tf_Baseline, x + l + rp->Font->tf_XSize / 2, y + 2 - rp->Font->tf_Baseline + rp->Font->tf_YSize);
		SetAPen(rp, of);
		Move(rp, x, y + 2);
		Text(rp, title, a);
	}

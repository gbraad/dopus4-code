...
void _DOpus_CreateGadgetBorders(struct DOpusRemember **key, int w, int h, struct Border **selborder, struct Border **unselborder, int dogear, int shine, int shadow)
{
	...
	*unselborder = NULL;
	...
	if((shine_data = LAllocRemember(key, 10 * sizeof(short), MEMF_CLEAR)) && (shadow_data = LAllocRemember(key, 10 * sizeof(short), MEMF_CLEAR)))
	{
		...
		if ((*selborder = LAllocRemember(key, sizeof(struct Border) * 2, MEMF_CLEAR)) && (*unselborder = LAllocRemember(key, sizeof(struct Border) * 2, MEMF_CLEAR)))
		{

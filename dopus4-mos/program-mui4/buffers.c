...
void findemptybuffer(int win)
{
	...
	if(!ok)
	{
		...
		for(a = 0; a < data_buffer_count[win]; a++)
		{
			...
			if(CompareDates(&oldestdir->dirstamp, &dir->dirstamp) < 0)
				oldestdir = dir;
			dir = dir->next;
		}
		dir = oldestdir;
...
struct DirectoryWindow *findbuffer(char *dirbuf, int win, int canchecklocks, int onlyreturn)
{
	...
	for(try = 0; try < 2; try++)
	{
		if(checklocks && !(lock = Lock(tempbuf, ACCESS_READ)))
			break;
		...
		for(a = 0; a < data_buffer_count[win]; a++, dir = dir->last)
		{
			if(dir->directory[0])
			{
				if(checklocks && (testlock = Lock(dir->directory, ACCESS_READ)))
				{
				...
				{
					if(!(lockandexamine(tempbuf, &fblock)) || (CompareDates(&fblock.fib_Date, &dir->dirstamp) != 0))
						continue;
					if(!onlyreturn)
...
void userentrymessage(struct DirectoryWindow *dir, struct Directory *entry, int type)
{
	...
	if(!(msg = CreateRexxMsg(general_port, NULL, str_arexx_portname)))
		fail = 1;
	else
	{
...
void check_old_buffer(int win)
{
	...
	if(config->dirflags & DIRFLAGS_REREADOLD && dopus_curwin[win]->directory[0])
	{
		...
		else
		{
			...
			{
				if(CompareDates(&dopus_curwin[win]->dirstamp, &testinfo.fib_Date) != 0)
				{
					reread = 1;
				}

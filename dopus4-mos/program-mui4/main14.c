...
//                a = Assign(buf,blank ? NULL : buf1);
	...
	for(a = 0; a < strlen(buf); a++)
		if(buf[a] == ':' || buf[a] == '/')
			buf[a] = 0;
	if(!(Assign(buf, str_pathbuffer[data_active_window])))
		okay();
	else
...
int checksame(STRPTR src, STRPTR dst, int type)
{
	BPTR lk1, lk2;
	int ret;

	if(!(lk1 = Lock(src, ACCESS_READ)))
...
int expand_path(STRPTR path, STRPTR buffer)
{
	BPTR lock;
	APTR save;
	int suc;

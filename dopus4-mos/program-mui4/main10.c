...
void dodevicelist(int win)
{
	...
	while((dl = NextDosEntry(dl, LDF_DEVICES)))
	{
		if(dl->dol_Type != DLT_DEVICE || dl->dol_Task)
		{
			...
			ULONG len = *s + 1;
			stccpy(devname, s+1, len > 32 ? 32 : len);
			strcat(devname, ":");
...
int huntfile(STRPTR name, STRPTR completename, int *aa)
{
	char buf[FILEBUF_SIZE];
	int rec;

	StrToUpper(name, buf);
	if(MatchPatternNoCase(str_hunt_name_parsed, buf))
	{
		char mesbuf[300], *ptr;

		if((ptr = FilePart(completename)))
		{
			strcpy(buf, FilePart(completename));
			*ptr = 0;
		}
		else
			buf[0] = 0;
...
char *parsedatetime(STRPTR buf, STRPTR dbuf, STRPTR tbuf, int *dis)
{
...
ULONG clone_screen(struct Screen *original, struct ExtNewScreen *clone)
{
	...
	if(original || (original = screen = LockPubScreen(NULL)))
	{
	...
	GetScreenData( /*(char *) */ &scrbuf, sizeof(struct Screen), (original) ? CUSTOMSCREEN : WBENCHSCREEN, original);
...
int copy_string(STRPTR string, STRPTR * copy, struct DOpusRemember **memkey)
{
	...
	if(!(*copy = LAllocRemember(memkey, strlen(string) + 1, 0)))
		return (0);
	strcpy(*copy, string);
	return (1);
}

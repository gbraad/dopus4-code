...
void dofunctionstring(STRPTR func, STRPTR name, STRPTR title, struct dopusfuncpar *pars)
{
	char fbuf[256], *ptr;
	...
	if(!func || !func[0] || !(funcdata = LAllocRemember(&general_key, sizeof(struct function_data), MEMF_CLEAR)))
		return;
	...
	{
		noloop = 1;
		if((ptr = FilePart(func_external_file)))
...
int handlefunctionss(STRPTR funcbuf, STRPTR name, STRPTR title, struct dopusfuncpar *pars, struct function_data *funcdata)
{
	...
	else
	{
		...
		{
			if(pars)
				CopyMem((char *)pars, (char *)&par, sizeof(struct dopusfuncpar));
			else
				defaultpar(&par);
			switch (funcbuf[0])
			{
...
int getfunction(STRPTR func, STRPTR *funcptr)
{
	int a, b;
...
//			if(strncmp(commandlist[b].name, &func[1], a) == 0 && (_isspace(func[a + 1]) || func[a + 1] == 0))
			if(Strnicmp(commandlist[b].name, &func[1], a) == 0 && (_isspace(func[a + 1]) || func[a + 1] == 0))
			{
				if(funcptr && func[a + 1])
					*funcptr = &func[a + 1];
	...
	for(pos = 0; pos < line_len; pos++)
	{
		...
		case FUNC_ONEPATH_NO:
			...
			else
			{
				...
				strcpy(funcdata->last_file, funcdata->source_path);
				TackOn(funcdata->last_file, func_single_file,
				       256);
			}
			break;
...
void custunselect(struct Directory *dir, int rel, struct function_data *funcdata)
{
	...
	if(!(status_flags & STATUS_FROMHOTKEY) && !func_external_file[0])
	{
		if(rel && dir->name[0])
		{
			if((temp = LAllocRemember(&funcdata->reload_memkey, sizeof(struct recpath), MEMF_CLEAR)) && (temp->path = LAllocRemember(&funcdata->reload_memkey, FILEBUF_SIZE, MEMF_CLEAR)))
			{
...
struct Directory *reload_file(int win, char *name)
{
	...
	{
		if((cust = findfile(dopus_curwin[win], FilePart(name), NULL)))
			removefile(cust, dopus_curwin[win], win, FALSE);
	}
	return (ret);
}
...
int openscriptfile(struct dopusfuncpar *par, struct function_data *funcdata)
{
	...
	for(a = 0; a < 100; a++)
	{
		...
		if((funcdata->output_file = Open(funcdata->scriptname, MODE_NEWFILE)))
			break;
	}
	if(!funcdata->output_file)
	{
	...
	if(par->which & FLAG_SHELLUP)
	{
		if(config->shellstartup[0])
			StrCombine(buf, "S:", config->shellstartup, 40);
		else
...
int closescriptfile(struct dopusfuncpar *par, int run, struct function_data *funcdata)
{
	...
	if(run > 0 && par && funcdata->output_file)
	{
		...
		if(flags & FLAG_OUTWIND && !wb2f && MainScreen)
		{
			...
			SetDefaultPubScreen(str_arexx_portname);
			setcust = 1;
		}

		if(flags & FLAG_OUTFILE && (tnil = Open(funcdata->tempfile, MODE_NEWFILE)))
			otemp = 1;
		if(!tnil)
			tnil = Open("NIL:", MODE_NEWFILE);
		...
		if(msgport && Window)
		{
			...
			FOREVER
			{
				if((bit = Wait(1 << msgport->mp_SigBit | rexx_signalbit | INPUTSIG_ABORT)) & INPUTSIG_ABORT)
				{
					status_justabort = status_haveaborted = 0;
					break;
				}
...
int getdummypath(char *dir, int title)
{
	...
	for(a = 0; a < len && pos < 255; a++)
	{
		if(string[a] == '[' && string[a + 2] == ']')
		{
			...
			case 'f':
				...
				break;
			case 'o':
				if((ptr = FilePart(filename)))
					LStrnCpy(&buffer[pos], ptr, 255 - pos);
				break;
			case 's':

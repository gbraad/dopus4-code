...
void drawrecaround(struct RastPort *r, int x, int y, int x1, int y1, int width, int height)
{
	...
	SetAPen(r, 0);
	if(x > 0)
		RectFill(r, 0, 0, x - 1, height - 1);
	if(x1 < width)
		RectFill(r, x1, 0, width - 1, height - 1);
	if(y > 0)
		RectFill(r, 0, 0, width - 1, y - 1);
	if(y1 < height)
		RectFill(r, 0, y1, width - 1, height - 1);
	SetAPen(r, o);
}
...
int doplay8svx(STRPTR fname, int loop)
{
	...
	if((o = NewDTObject(fname, DTA_GroupID, GID_SOUND, TAG_END)))
	{
		if((sig = AllocSignal(-1)) != -1)
		{
			SetAttrs(o, SDTA_SignalTask, FindTask(NULL), SDTA_SignalBit, 1L << sig, DTA_Repeat, loop ? TRUE : FALSE, TAG_END);
...
int doplay8svxold(STRPTR fname, int loop)
{
	...
	USHORT code;
	ChunkHeader *p8chunk;
	ULONG a, b, stereo, *vxcheck, finish, playsize = 0;
	...
	if(vhdr)
	{
		...
		{
			size -= 2;
			if(!(compressbuf = LAllocRemember(&audio_key, size * 2, MEMF_ANY)))
				return (-2);
...

	{
		if(OpenAHI())
		{
			AHISoundHook.h_Entry = (HOOKFUNC)&HookEntry;
			AHISoundHook.h_SubEntry = (HOOKFUNC)AHISoundFunc;
			if((actrl = AHI_AllocAudio(AHIA_AudioID, AHI_DEFAULT_ID, AHIA_MixFreq, AHI_DEFAULT_FREQ, AHIA_Channels, stereo ? 2 : 1, AHIA_Sounds, stereo ? 2 : 1, loop ? TAG_IGNORE : AHIA_SoundFunc, (Tag)&AHISoundHook, AHIA_UserData, (Tag) FindTask(NULL), TAG_DONE)))
			{
				...
				if((snd0 != AHI_NOSOUND) && (snd1 != AHI_NOSOUND))
				{
					if((AHIsignal = AllocSignal(-1)) != -1)
					{
						...
						AHI_Play(actrl, AHIP_BeginChannel, 0, AHIP_Freq, vhdr ? vhdr->vh_SamplesPerSec : 10000, AHIP_Vol, vhdr ? vhdr->vh_Volume : Unity, AHIP_Pan, stereo ? Unity : pan, AHIP_Sound, 0, AHIP_EndChannel, NULL, stereo ? TAG_MORE : TAG_END, (Tag) ahitags2);
					}
					else
						CloseAHI();
				}
...

	{
		...
		FOREVER
		{
			if(actrl == NULL)
			{
				...
				size -= playsize;
				for(a = 0; a < 2; a++)
					BeginIO((struct IORequest *)audio_req1[a]);
				for(a = 0; a < 2; a++)
					BeginIO((struct IORequest *)audio_req2[a]);
				a = 0;
			}
			FOREVER
			{
				...
				if(actrl == NULL)
				{
					...
					if(a == 3)
					{
						...
						else
						{
							...
							psample += playsize / 2;
							for(b = 0; b < 2; b++)
								BeginIO((struct IORequest *)audioptr[b]);
						}
						a = 0;
					}
				}
...
void dosound(int type)
{
	if(type)
	{
...

		{
			if(OpenAHI())
			{
				if((actrl = AHI_AllocAudio(AHIA_AudioID, AHI_DEFAULT_ID, AHIA_MixFreq, AHI_DEFAULT_FREQ, AHIA_Channels, 1, AHIA_Sounds, 1, TAG_DONE)))
				{
					...
					if(AHI_LoadSound(0, AHIST_SAMPLE, &sample, actrl) != AHI_NOSOUND)
					{
						...
						for(a = 0; a < 11; a++)
						{
							AHI_Play(actrl, AHIP_BeginChannel, 0, AHIP_Freq, (a % 2) ? 6000 : 9000, AHIP_Vol, 0x10000L, AHIP_Pan, 0x8000L, AHIP_Sound, 0, AHIP_EndChannel, NULL, TAG_END);
							Delay(6);
						}

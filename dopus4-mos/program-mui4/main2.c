...
int getdir(struct DirectoryWindow *dir, int win, int incmess)
{
	...
	if(!(mylock = Lock(dir->directory, ACCESS_READ)))
	{
		...
		{
			LStrnCpy(dir->realdevice, str_pathbuffer[win], a + 1);
		}
		return (0);
	}
		...
		while(exall_continue)
		{
			...
			while(exall_current)
			{
				...
				if(exall_current->ed_Comment)
				{
					...
					stccpy(commentbuf, exall_current->ed_Comment, 80);
				}
				else
				{
				...
				if(exall_current->ed_Type == ST_SOFTLINK || exall_current->ed_Type == ST_LINKDIR || exall_current->ed_Type == ST_LINKFILE)
				{
...

					{
						...
						{
							if(ReadLink(dp->dvp_Port, ld, fileinfo.fib_FileName, buf, 256))
							{
...
void buildkmgstring(char *buf, UQUAD size, int kmgmode)
{
	if(kmgmode)
	{
		if(size > 1024)
...
/*
void buildkmgstringnew(char *buf, UQUAD size, int kmgmode, uint32 type)
{
	if(kmgmode)
	{
		if(size > 1024)
...
//				buildkmgstringnew(sizebuf, display->size, config->listerdisplayflags[win] & SIZE_KMG, display->subtype);
			...
			for(a = 0; a < DISPLAY_LAST + 1; a++)
			{
				...
				case DISPLAY_PROTECT:
					...
					{
						w = TextLength(&dir_rp[win], sptr, b = 8);
						for(; *sptr;)
							*sbuf++ = *sptr++;
					}

...
void view_print(struct ViewData *, STRPTR, int, int);
...
void view_makeuphex(struct ViewData *, STRPTR, unsigned char *, int);
...
int view_whatsit(struct ViewData *, STRPTR, int, STRPTR);
...
{
	0, 0, 0, 0, 255, 255, IDCMP_RAWKEY | IDCMP_VANILLAKEY | IDCMP_MOUSEBUTTONS | IDCMP_GADGETUP | IDCMP_GADGETDOWN | IDCMP_MOUSEMOVE, 0, NULL, NULL, NULL, NULL, NULL, 0, 0, 0, 0, CUSTOMSCREEN
};
...
static void view_file_process()
{
	...
	if((view_port = CreateMsgPort()))
	{
		view_port->mp_Node.ln_Name = portname;
		AddPort(view_port);
			...
			if(size > 0)
			{
				...
				if(vdata)
				{
					...
					if(config->viewbits & VIEWBITS_INWINDOW)
						SetWindowTitles(vdata->view_window, titlebuf, "Directory Opus Reader");
					else
					...
					if(view_loadfile(vdata))
					{
						...
						{
							GT_SetGadgetAttrs(viewGadgets[VIEW_LINECOUNT], vdata->view_window, NULL, GTNM_Number, vdata->view_line_count, TAG_END);
...
int view_loadfile(struct ViewData *vdata)
{
	...
	for(a = 0; a < 2; a++)
	{
		...
		if((vdata->view_buffer_size = AvailMem(MEMF_LARGEST)) < 16)
			break;
	...
	if(!vdata->view_text_buffer)
		return 0;;
	if(!(in = Open(vdata->view_path_name, MODE_OLDFILE)))
		return 0;
...
int view_setupdisplay(struct ViewData *vdata)
{
	...
	UWORD zoom[4] = { ~0, ~0, config->viewtext_width, config->viewtext_height };
	...
	if(config->viewbits & VIEWBITS_INWINDOW)
	{
		vdata->view_window = OpenWindowTags(&viewwin, WA_AutoAdjust, TRUE, WA_Zoom, (ULONG)&zoom, TAG_END);
	}
	else
	{
	...
	RectFill(vdata->view_rastport, vdata->view_display_left, vdata->view_display_top, vdata->view_display_right, vdata->view_display_bottom);
	...
	{
		/* text display border */
		Do3DBox(vdata->view_rastport, vdata->view_display_left, vdata->view_display_top - 1, (vdata->view_display_right - vdata->view_display_left) + 2, vdata->view_display_bottom - vdata->view_display_top + 2, vdata->view_colour_table[PEN_SHINE], vdata->view_colour_table[PEN_SHADOW]);
		/* status border */
		Do3DBox(vdata->view_rastport, vdata->view_display_left, vdata->view_status_bar_ypos, width - ((fontx * 7 * 2) + 4), vdata->view_font->tf_YSize, vdata->view_colour_table[PEN_SHINE], vdata->view_colour_table[PEN_SHADOW]);
		/* status bar */
		SetAPen(vdata->view_rastport, vdata->view_colour_table[VIEWPEN_STATUSBACKGROUND]);
		RectFill(vdata->view_rastport, vdata->view_display_left, vdata->view_status_bar_ypos, vdata->view_window->Width - ((config->viewbits & VIEWBITS_INWINDOW) ? vdata->view_window->BorderRight : 0) - ((fontx * 7 * 2) + 3), vdata->view_window->Height - 2 - ((config->viewbits & VIEWBITS_INWINDOW) ? vdata->view_window->BorderBottom : 0));
...
void view_displayall(struct ViewData *vdata)
{
	...
	SetAPen(vdata->view_rastport, vdata->view_colour_table[PEN_TEXTBACKGROUND]);
	RectFill(vdata->view_rastport, vdata->view_display_left, vdata->view_display_top, vdata->view_display_right, vdata->view_status_bar_ypos - 3);
...
void view_print(struct ViewData *vdata, STRPTR str, int y, int len)
{
	...
	if(vdata->view_display_as_ansi)
	{
		if(len > 0)
		{
			...
			if(y > -1)
			{
				ClipBlit(vdata->view_ansiread_window->RPort, 0, 0, vdata->view_rastport, vdata->view_display_left, vdata->view_display_top + (y * vdata->view_font->tf_YSize), vdata->view_ansiread_window->Width, vdata->view_font->tf_YSize, 0xc0);
			}
		}
	}
	else
	{
		Move(vdata->view_rastport, vdata->view_display_left, vdata->view_display_top + (y * vdata->view_font->tf_YSize) + vdata->view_font->tf_Baseline);
		...
		if(len > 0)
			Text(vdata->view_rastport, (char *)textbuf, len);
	}
}
...
void view_update_status(struct ViewData *vdata)
{
	if(config->viewbits & VIEWBITS_TEXTBORDERS)
	{
		...
		GT_SetGadgetAttrs(viewGadgets[VIEW_JUMPTOLINE1], vdata->view_window, NULL, GTNM_Number, vdata->view_text_offset, TAG_END);
		GT_SetGadgetAttrs(viewGadgets[VIEW_JUMPTOLINE2], vdata->view_window, NULL, GTNM_Number, (vdata->view_text_offset + vdata->view_lines_per_screen > vdata->view_line_count) ? vdata->view_line_count : vdata->view_text_offset + vdata->view_lines_per_screen, TAG_END);
		GT_SetGadgetAttrs(viewGadgets[VIEW_JUMPTOPERCENTNUM], vdata->view_window, NULL, GTNM_Number, a, TAG_END);

		a = vdata->view_line_count >> 15;
		a++;
		GT_SetGadgetAttrs(viewGadgets[VIEW_SCROLLGADGET], vdata->view_window, NULL, GTSC_Top, vdata->view_text_offset / a, GTSC_Total, vdata->view_line_count / a, GTSC_Visible, vdata->view_lines_per_screen / a, TAG_END);
	}
}
...
void view_search(struct ViewData *vdata, int ask)
{
	...
	if(LStrCmpI(temp, vdata->view_search_string))
		view_clearsearch(vdata);
...
void view_doscroll(struct ViewData *vdata, int dy, int w)
{
	...
	ScrollRaster(vdata->view_rastport, 0, dy, vdata->view_display_left, vdata->view_display_top, vdata->view_display_left + w, vdata->view_display_top + vdata->view_display_height - 1);
	if(!vdata->view_display_as_ansi)
...
void view_status_text(struct ViewData *vdata, char *str)
{
	...
	{
		strcpy(buf, str);
		GT_SetGadgetAttrs(g, vdata->view_window, NULL, GTTX_Text, (Tag) buf, TAG_END);
	}
}
...
//		dopus_print(0, &print_args, 0, vdata->view_port_name, vdata);
	...
	if(!vdata->view_display_as_hex)
	{
		...
		{
			Close(out);
			sprintf(temp, globstring[STR_ERROR_PRINTING_FILE], IoErr());
...
void view_viewhilite(struct ViewData *vdata, int x, int y, int x1, int y1)
{
	...
	if(!(hi = AllocMem(sizeof(struct viewhilite), MEMF_CLEAR)))
		return;

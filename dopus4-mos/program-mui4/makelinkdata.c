...
STATIC CONST struct TagItem makelink_type_gadget[] =
{
...
int getmakelinkdata(char *namebuf, char *destbuf, int *type)
{
	...
	if(!(swindow = OpenRequester(&makelink_req)) || !(name_gad = addreqgadgets(&makelink_req, makelink_gadgets, 0, NULL)) || !(AddRequesterObject(&makelink_req, makelink_type_text)) || !(AddRequesterObject(&makelink_req, makelink_name_text)) || !(AddRequesterObject(&makelink_req, makelink_dest_text)))
	{
				...
				case MAKELINK_DESTSELECT:
					{
						...
						strcpy(dirbuf, makelink_destbuf);
						...
						{
							dirbuf[0] = 0;
							strcpy(filebuf, makelink_destbuf);
						}
						else
							filebuf[0] = 0;
						if(FileRequest(&filereq))
						{
							strcpy(makelink_destbuf, dirbuf);

...
void makedir(int rexx)
{
	...
	FOREVER
	{
		...
		{
			doerror((err = IoErr()));
			if(rexx || (a = checkerror(globstring[STR_CREATING_DIRECTORY], FilePart(new_directory), -err)) == 3)
			{
				myabort();
				return;
			}
...
int iconwrite(int type, CONST_STRPTR name)
{
	int suc, err;
	...
	FOREVER
	{
		...
		{
			if((diskobj = GetDefDiskObject(type + 2)))
			{
			...
			else
			{
				...
				if(PutDiskObject(namebuf, icontable[type]))
					return (1);
				err = IoErr();
			}
		}
		if((suc = checkerror(globstring[STR_ADDING_ICON], FilePart(namebuf), err)) == 3)
			return (-1);
		if(suc == 0 || suc == 2)
			return (0);
	}
...
int copyicon(CONST_STRPTR srce, CONST_STRPTR dest, int *err)
{
	int suc;
	struct DiskObject *diskobj;
	char buf[256], *ptr;
	...
	if((diskobj = GetDiskObject(buf)))
	{
		suc = PutDiskObject(dest, diskobj);
		FreeDiskObject(diskobj);
		if(!suc || CheckExist(dest, NULL) >= 0)
			*err = IoErr();
	}
	else
	{
...
const char *isicon(CONST_STRPTR name)
{
	int a;
...
char *getarexxpath(int rexx, int win, int num, int argnum)
{
	...
	if((strchr(rexx_args[argnum], ':') || strchr(rexx_args[argnum], '/')) && (a = CheckExist(rexx_args[argnum], &b)))
	{
		if(!num)
		{
...
int readfile(CONST_STRPTR name, STRPTR *buf, ULONG *size)
{
	int in, retval  = 0;

	if(CheckExist(name, (int *)size) >= 0 || !(in = Open(name, MODE_OLDFILE)))
		return (-1);
...
int checkscreenmode(ULONG mode)
{
	...
	if(!(handle = FindDisplayInfo(mode)) || !(GetDisplayInfoData(handle, buf, 256, DTAG_DISP, 0)) || ((struct DisplayInfo *)buf)->NotAvailable)
		return (MODE_WORKBENCHCLONE);

	flags = ((struct DisplayInfo *)buf)->PropertyFlags;

	if(flags & DIPF_IS_DUALPF || flags & DIPF_IS_PF2PRI || flags & DIPF_IS_HAM || !(GetDisplayInfoData(handle, buf, 256, DTAG_DIMS, 0)))
		return (MODE_WORKBENCHCLONE);
...
void doarrowgadgets(struct Gadget *gad, int dir)
{
	...
	for(a = 0; a < 2; a++)
	{
		...
		DoArrow(main_rp, gad->LeftEdge + 2, gad->TopEdge + 1, gad->Width - 4, gad->Height - 2, screen_pens[config->arrowfg].pen, screen_pens[config->arrowbg].pen, dir);
		gad = gad->NextGadget;
	}
}
...
void dolittlegads(struct Gadget *gad, CONST_STRPTR txt, int num)
{
	int a, y;

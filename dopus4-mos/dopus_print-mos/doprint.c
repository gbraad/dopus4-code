...
int printfile(struct RequesterBase *reqbase, char *filename, PrintData *printdata, struct Requester *requester)
{
	...
	for(;;)
	{
		if(CheckExist(filename, &filesize) < 0)
			break;
		if(filesize == 0)
			return 0;
	...
	if(!(datetime = LAllocRemember(&memkey, sizeof(struct DateTime), MEMF_CLEAR)) || !(handle = LAllocRemember(&memkey, sizeof(struct PrintHandle), MEMF_CLEAR)))
	{
	...
	handle->filename = FilePart(filename);
	...
	if(Request(requester, reqbase->rb_window))
	{
		...
		Do3DBox(handle->req_rp, 2, 1, requester->Width - 4, requester->Height - 2, reqbase->rb_shine, reqbase->rb_shadow);
	...
	if(printdata->print_flags & PRINTFLAG_FILE && printdata->output_file[0])
	{
		...
		for(;;)
		{
			if((handle->filehandle = Open(printdata->output_file, (a = (CheckExist(printdata->output_file, NULL)) ? MODE_READWRITE : MODE_NEWFILE))))
			{
				if(a == MODE_READWRITE)
					Seek(handle->filehandle, 0, OFFSET_END);
				handle->fileoutput = 1;
				break;
			}
		...
		a = LOCK_DIFFERENT;
		src_lock = Lock(filename, ACCESS_READ);
		dst_lock = Lock(printdata->output_file, ACCESS_READ);
		if(src_lock && dst_lock)
			a = SameLock(src_lock, dst_lock);
		UnLock(dst_lock);
		UnLock(src_lock);
		if(a == LOCK_SAME)
		{
	...
	else
	{
		...
		if(!(handle->port = CreateMsgPort()) || !(handle->ioreq = (struct IOStdReq *)CreateIORequest(handle->port, sizeof(struct IOStdReq))))
			goto ENDPRINT;

		for(;;)
		{
			if(!(OpenDevice("printer.device", 0, (struct IORequest *)handle->ioreq, 0)))
				break;
			if(!(check_error(reqbase, string_table[STR_UNABLE_TO_OPEN_PRINTER], 0)))
			{
				DeleteIORequest((struct IORequest *)handle->ioreq);
				handle->ioreq = NULL;
				goto ENDPRINT;
			}
		}
	...
	if(handle->req_rp)
	{
		...
		RectFill(handle->req_rp, 2, 1, requester->Width - 3, requester->Height - 2);
						...
						default:
							...
							if(pos > handle->paper_width)
							{
								...
								if(lastspace > 0 && lastspace < pos)
								{
									...
									pos -= a;
									if(pos > 0)
										CopyMem(&linebuffer[a], linebuffer, pos);
								}
								else
								{
	...
	DeleteMsgPort(handle->port);
	if(handle->req_rp)
		EndRequest(requester, reqbase->rb_window);
	LFreeRemember(&memkey);
	return (ret);
}
...
printer_command(struct RequesterBase *reqbase, struct PrintHandle *handle, char *command)
{
	...
	for(;;)
	{
		if(!(DoIO((struct IORequest *)handle->ioreq)))
			break;
		if(!(check_error(reqbase, string_table[STR_UNABLE_TO_OPEN_PRINTER], 0)))
			return (0);
	}
...
void print_status(struct PrintHandle *handle, char *text, int y)
{
	...
	{
		SetAPen(handle->req_rp, reqbase->rb_bg);
		RectFill(handle->req_rp, 2, y, x, y + font->tf_YSize);
	}
	SetAPen(handle->req_rp, reqbase->rb_fg);
	...
	{
		SetAPen(handle->req_rp, reqbase->rb_bg);
		RectFill(handle->req_rp, handle->req_rp->cp_x, y, handle->req_width - 3, y + font->tf_YSize);
	}
	SetAPen(handle->req_rp, reqbase->rb_fg);
}

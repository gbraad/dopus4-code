...
void do_printdir(struct VisInfo *vis, char *portname, int wind)
{
	...
	if(!(window = OpenRequester(&printdirreq)) || !(addreqgadgets(&printdirreq, printdir_gadgets, gadgets)) || !(AddRequesterObject(&printdirreq, printdir_print_text)) || !(AddRequesterObject(&printdirreq, printdir_output_text)))
	{
...
void printdirectory(struct RequesterBase *reqbase, char *portname, int flags, char *output, int wind)
{
	...
	while(!out)
	{
		if(out = Open(output, MODE_NEWFILE))
			break;

...
{
	42, 9, 92, 64, 8, 8, 0, 0, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, NULL, IDCMP_GADGETUP | IDCMP_VANILLAKEY | IDCMP_MOUSEBUTTONS, NULL
};

static struct TagItem protect_bits[8][9];

struct TagItem protect_bit_template[] =
{
...
struct TagItem *protect_gadgets[] =
{
	protect_bits[0],
	protect_bits[1],
	protect_bits[2],
...
int getprotectdata(int *prot, int *mask)
{
	...
	for(a = 0; a < 8; a++)
		prot_template[a] = str_protbits[7 - a];
	prot_template[8] = 0;
	...
	for(a = 0; a < 8; a++)
	{
		CopyMem((char *)protect_bit_template, (char *)protect_bits[a], sizeof(protect_bit_template));
	...
	if(!(pwindow = OpenRequester(&protect_req)) || !(gadlist = addreqgadgets(&protect_req, protect_gadgets, 0, NULL)) || !(old_text = AddRequesterObject(&protect_req, protect_old_text)) || !(new_text = AddRequesterObject(&protect_req, protect_new_text)) || !(mask_text = AddRequesterObject(&protect_req, protect_mask_text)))
	{
	...
	FOREVER
	{
		while((IMsg = (struct IntuiMessage *)GetMsg(pwindow->UserPort)))
		{
			...
			case IDCMP_MOUSEBUTTONS:
				if(code == SELECTDOWN && x >= mask_text->ob_left && x < (mask_text->ob_left + mask_text->ob_width) && y >= mask_text->ob_top && y < (mask_text->ob_top + mask_text->ob_height))
				{
			...
			case IDCMP_VANILLAKEY:
				...
				else if((code == getkeyshortcut(globstring[STR_CANCEL])) || (code == 0x1B))
			...
			case IDCMP_GADGETUP:
				...
				case PROTECT_CANCEL:
					CloseRequester(&protect_req);
					return ((gadgetid == PROTECT_CANCEL) ? 0 : ((gadgetid == PROTECT_ALL) ? 2 : 1));
				}
				break;
			}
		}
...
void do_prot_display(int bits, Object_Border *border, struct Gadget *gadgets)
{
	char bitbuf[10];
	int gadnum;
	struct Gadget *gad;
	...
	ObjectText(&protect_req, border->ob_left, border->ob_top, border->ob_width, border->ob_height, bitbuf, TEXTPOS_CENTER);

...
int do_parent_multi(STRPTR path)
{
	...
	if(!(rwindow = OpenRequester(&pm_req)) || !(gadlist = addreqgadgets(&pm_req, pm_gadgets, 0, NULL)) || !(listview = (struct DOpusListView *)AddRequesterObject(&pm_req, pm_lister)))
	{
	...
	for(;;)
	{
		while((IMsg = (struct IntuiMessage *)GetMsg(rwindow->UserPort)))
		{
			...
			micros = IMsg->Micros;
			if((view = (struct DOpusListView *)ListViewIDCMP(listview, IMsg)) == (struct DOpusListView *)-1)
			{
			...
			else if(view)
			{
				if(table[view->itemselected])
				{
					...
					if(view->itemselected == lastsel && (DoubleClick(lastseconds, lastmicros, seconds, micros)))
					{
						RemoveListView(listview, 1);
						CloseRequester(&pm_req);
						strcpy(path, buf);

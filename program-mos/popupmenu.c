...
void handlelistermenu(int a)
{
	...
	MenuHandler.h_Entry = (HOOKFUNC)&HookEntry;
	MenuHandler.h_SubEntry = (HOOKFUNC) MenuHandlerFunc;
	MenuHandler.h_Data = &sortorder;

	for(r = DISPLAY_NAME; r <= DISPLAY_LAST; r++)
		PM_SetItemAttrs(PM_FindItem(sortmenu, 1 + r), PM_Checked, config->sortmethod[a] == r, TAG_END);

	PM_SetItemAttrs(PM_FindItem(sortmenu, sbReverse), PM_Checked, oldsort, TAG_END);

	for(r = 0; r < 3; r++)
		PM_SetItemAttrs(PM_FindItem(sortmenu, (r + 1) * 0x00010000), PM_Checked, config->separatemethod[a] == r, TAG_END);

	for(r = 0; r < 3; r++)
		PM_SetItemAttrs(PM_FindItem(sortmenu, (r + 1) * 0x01000000), PM_Checked, SORT_NAMEMODE(config->sortflags) == r, TAG_END);

	PM_SetItemAttrs(PM_FindItem(sortmenu, sbKMG), PM_Checked, (config->listerdisplayflags[a] & SIZE_KMG) == SIZE_KMG, TAG_END);	// HUX
	...
	PM_OpenPopupMenu(Window, PM_Menu, sortmenu, PM_MenuHandler, &MenuHandler, PM_ForceFont, scr_font[FONT_MENUS], TAG_END);

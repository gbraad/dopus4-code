...
char stringmap[] = { STR_FORMAT_NAME, STR_FORMAT_FFS, STR_FORMAT_INTERNATIONAL, STR_FORMAT_CACHING,
                     STR_FORMAT_TRASHCAN, STR_FORMAT_VERIFY, STR_FORMAT_FORMAT, STR_FORMAT_QUICKFORMAT,
                     STR_FORMAT_EXIT };
/* prototypes */
void show_device_info(struct RequesterBase *reqbase,Object_Border *border,char *name);
...
char *argv[];
{
    ...
    if (formatreq.rb_screen && !(vis->vi_flags&VISF_BORDERS)) {
        ...
        formatreq.rb_title=string_table[STR_WINDOW_FORMAT];
    }
    ...
    for(a = 0; a < 9; a++) keys[a] = getkeyshortcut(string_table[stringmap[a]]);

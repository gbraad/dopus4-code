...
// JRZ: begin
        ...
        if (src_lock && dst_lock) a = SameLock(src_lock,dst_lock);
        UnLock(dst_lock);
        UnLock(src_lock);
        if (a == LOCK_SAME)
         {
//            kprintf("Printing to source file!\n");
          goto ENDPRINT;
         }
// JRZ: end
    }
...
/* JRZ
    if (PPBase) {
        FOREVER {
            if ((a = ppLoadData(filename,DECR_POINTER,MEMF_CLEAR,(UBYTE **)&buffer,&buffersize,NULL))==PP_PASSERR) {

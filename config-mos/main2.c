...
int dogadgetconfig()
{
	...
	if(!(gadgads = (struct Gadget *)LAllocRemember(&gadkey, sizeof(struct Gadget) * GADCOUNT, MEMF_CLEAR)))
		return (0);

	CreateGadgetBorders(&gadkey, 80, 10, &selborder, &unselborder, 0, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
...
void showgadgetname(struct newdopusfunction *gad, struct Gadget *gadget)
{
	...
	if(gad)
		SetAPen(rp, screen_pens[gad->bpen].pen);
	else
...
void dogadgetinfo(STRPTR str)
{
	...
	Do3DBox(rp, x, y_off + 144, 580, 10, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	SetDrMd(rp, JAM2);
	...
	SetAPen(rp, screen_pens[0].pen);
	if(tx > x)
		RectFill(rp, x, y_off + 145, tx - 1, y_off + 153);
	if(ex < x + 579)
		RectFill(rp, ex, y_off + 145, x + 579, y_off + 153);
}
...
int domenuconfig()
{
	...
	if(!(menugads = (struct Gadget *)LAllocRemember(&menukey, sizeof(struct Gadget) * (MENUCOUNT + 5 - (HIDDENLINES * 5)), MEMF_CLEAR)))
		return (0);
	selgad = NULL;
	selbank = NULL;
	menuoffset = 0;
	...
	AddGadgetBorders(&menukey, &menuslidergads[1], 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
								...
								else
								{
									...
									case MENU_COPYMENU:
										if(omnum < 0 || mnum == omnum)
											break;
										CopyMem((char *)&config->menu[omnum * 20], (char *)&config->menu[mnum * 20], sizeof(struct newdopusfunction) * 20);
										b = (mnum * 20) + 20;
										for(a = mnum * 20; a < b; a++)
										{

...
int doscreenconfig()
{
	...
	for(;;)
	{
		...
		{
			if(!(mode == SCREEN_SCREENMODE && (view = ListViewIDCMP(&screenmodeview, IMsg)) != (struct DOpusListView *)-1) && !(mode == SCREEN_FONTS && (view = ListViewIDCMP(&fontplacelist, IMsg)) != (struct DOpusListView *)-1) && !(mode == SCREEN_PALETTE && (view = ListViewIDCMP(&palettelist, IMsg)) != (struct DOpusListView *)-1))
			{
				class = IMsg->Class;
				code = IMsg->Code;
				qual = IMsg->Qualifier;
				...
				case IDCMP_GADGETDOWN:
					...
					else if(gadgetid >= ARROWBASE + 3 && gadgetid < ARROWBASE + 7)
					{
						...
						for(;;)
						{
							if (IntuitionBase->ActiveWindow != Window || !(gad->Flags & GFLG_SELECTED))
								break;
			...
			else if(view)
			{
				...
				else if(view->listid == 1)
				{
					...
					if(curmode->mode == MODE_PUBLICSCREENUSE)
					{
						...
						{
							stccpy(config->pubscreen_name, curmode->name, a);
...
void makescreenedit(int mode)
{
	...
	case SCREEN_PALETTE:
		...
		if((handle = FindDisplayInfo(config->screenmode)) || (handle = FindDisplayInfo(modeid)))
		{
		...
		AddGadgets(Window, palettegads, palettegadgets, 6, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	...
	case SCREEN_COLOURS:
		...
		AddGadgetBorders(&borderkey, &coloursgad, 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
		AddGadgets(Window, &coloursgad, NULL, 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	...
	case SCREEN_SCREENMODE:
		...
		wclone = 0;
		busy();
		if((a = initscreenmodes()) && (screenmodelist = LAllocRemember(&screenkey, (a + 1) * 4, MEMF_CLEAR)))
		{
			...
			for(b = 0; b < a; b++)
			{
				...
				if(screenmode->mode == config->screenmode)
				{
					if(screenmode->mode == MODE_PUBLICSCREENUSE)
					{
						...
						if(Stricmp(buf, screenmode->name) == 0)
		...
		{
			if ((a + 300) != SCREENMODE_DEPTH && (a + 300) != SCREENMODE_SLIDER)
		...
		#endif
		AddGadgets(Window, screenmodegads, screenmodegadgets, 7, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
		Do3DBox(rp, x_off + 238, y_off + 140, 272, 40, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
		SetAPen(rp, screen_pens[1].pen);
		UScoreText(rp, cfg_string[STR_DISPLAY_MODE_DESCRIPTION], x_off + 238, y_off + 136, -1);
	...
	case SCREEN_FONTS:
		if(!fontdatabuf)
		{
			...
			if(!num || !(fontlist = LAllocRemember(&fontkey, (num + 1) * 4, MEMF_CLEAR)) || !(fontsizelist = LAllocRemember(&fontkey, (num + 1) * 4, MEMF_CLEAR)))
				break;
			if(!(noproplist = LAllocRemember(&fontkey, (num + 1) * 4, MEMF_CLEAR)) || !(nopropsizelist = LAllocRemember(&fontkey, (num + 1) * 4, MEMF_CLEAR)))
			{
				noproplist = fontlist;
			...
			for(fnum = 0, fontnum = 0, nopropnum = 0, only8num = 0; fnum < num; fnum++)
			{
				...
				if(fontsizelist[a])
				{
					for(b = 0;; b++)
					{
						if(!fontsizelist[a][b])
						{
							if((fontsizelist[a][b] = LAllocRemember(&fontkey, 8, MEMF_CLEAR)))
		...
		SetAPen(rp, screen_pens[1].pen);
		UScoreText(rp, cfg_string[STR_DISPLAY_ITEM], x_off + 146, y_off + 14, -1);
		UScoreText(rp, cfg_string[STR_FONT], x_off + 360, y_off + 14, -1);
		UScoreText(rp, cfg_string[STR_FONT_SIZE], x_off + 566, y_off + 14, -1);
		fontsize_buf[0] = 0;
		AddGadgets(Window, &fontsizegadget, NULL, 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
	...
	case SCREEN_ARROWS:
		...
		AddGadgetBorders(&borderkey, &arrowgadgets[3], 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
		AddGadgetBorders(&borderkey, &arrowgadgets[5], 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
		AddGadgetBorders(&borderkey, &arrowgadgets[7], 1, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);

		AddGadgets(Window, arrowgadgets, arrowgadtxt, 8, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
		for(a = 0; a < 2; a++)
		{
		...
		fixarrowgads(0);
		SetAPen(rp, screen_pens[1].pen);
		UScoreText(rp, cfg_string[STR_MODIFY_SIZE], x_off + 294, y_off + 151, -1);
	...
	case SCREEN_SLIDERS:
		for(a = 0; a < 2; a++)
		{
			AddGadgetBorders(&borderkey, &screen_sliders_gadgets[a], 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen);
		}

		AddGadgets(Window, screen_sliders_gadgets, NULL, 2, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
...
void removescreenedit(int mode)
{
	...
	RectFill(rp, x_off + 120, y_off + 4, x_bot, y_bot);
	SetAPen(rp, screen_pens[1].pen);
	switch (mode)
	{
	case SCREEN_PALETTE:
	...
	case SCREEN_FONTS:
		...
		RemoveGList(Window, &fontsizegadget, 1);

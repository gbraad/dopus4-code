...
int initfontlist(int num, int move, int flags)
{
	...
	if(!fontlist)
		return (0);
	if(!flags)
		fontlistview.items = fontlist;
	else if(flags == FFLAG_8ONLY)
	...
	for(a = 0;; a++)
	{
		...
		if(strcmp(name, fontlistview.items[a]) == 0)
		{
			...
			{
				if(!flags)
					fontsizelistview.items = fontsizelist[a];
				else if(flags == FFLAG_8ONLY)
					fontsizelistview.items = only8sizelist;
				else if(flags == FFLAG_NOPROP)
					fontsizelistview.items = nopropsizelist[a];
				if(fontsizelistview.items)
				{
					one = -1;
					for(b = 0;; b++)
					{
						if(!fontsizelistview.items[b])
							break;
						if((val = atoi(fontsizelistview.items[b])) == (int)config->fontsizes[num])
						{
							...
							if(move)
							{
								if(b < fontsizelistview.topitem || b > fontsizelistview.topitem + fontsizelistview.lines)
...
void sortfontlist(struct AvailFonts *avail, int num, int type)
{
	int gap, i, j, k;
	...
	for(gap = num / 2; gap > 0; gap /= 2)
		for(i = gap; i < num; i++)
			for(j = i - gap; j >= 0; j -= gap)
			{
				...
				if(type == 0)
				{
					if(LStrCmpI((char *)avail[j].af_Attr.ta_Name, (char *)avail[k].af_Attr.ta_Name) <= 0)
						break;
				}
				else
				{
					if(LStrCmpI((char *)avail[j].af_Attr.ta_Name, (char *)avail[k].af_Attr.ta_Name))
						break;
...
void dofontdemo(STRPTR name, int size)
{
	...
	clip.MaxX = x_off + 611;
	clip.MaxY = y_off + 167;
	Do3DBox(rp, x_off + 146, y_off + 93, 468, 76, screen_pens[config->gadgetbotcol].pen, screen_pens[config->gadgettopcol].pen);
	SetAPen(rp, screen_pens[0].pen);
	RectFill(rp, x_off + 146, y_off + 93, x_off + 613, y_off + 167);
	SetAPen(rp, screen_pens[1].pen);
	busy();
	UScoreText(rp, cfg_string[STR_LOADING_FONT], x_off + 329, y_off + 133, -1);
	if(font = getfont(name, &size, 0))
	{
		SetAPen(rp, screen_pens[0].pen);
		RectFill(rp, x_off + 146, y_off + 94, x_off + 613, y_off + 167);
		SetAPen(rp, screen_pens[1].pen);
		SetFont(rp, font);
		if((reg = NewRegion()))
		{
		...
		{
			if(isupper(buf[0][a]))
				buf[1][a] = ToLower(buf[0][a]);
			else
				buf[1][a] = ToUpper(buf[0][a]);
		}
		a = 0;
		...
		UScoreText(rp, cfg_string[STR_FONT_COULD_NOT_BE_LOADED], x_off + 289, y_off + 133, -1);
	unbusy();
}

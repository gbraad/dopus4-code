...
dodriveconfig()
{
	...
	gadkey = NULL;
	if(!(drivegads = (struct Gadget *)LAllocRemember(&gadkey, sizeof(struct Gadget) * USEDRIVECOUNT, MEMF_CLEAR)))
		return (0);
		...
		while(IMsg = getintuimsg())
		{
			...
			case IDCMP_GADGETUP:
							...
							else
							{
								...
								case DRIVE_COPYBANK:
									if(dnum != odnum)
									{
										for(a = 0; a < 6; a++)
										{
											CopyMem((char *)&config->drive[(odnum * 6) + a], (char *)&config->drive[(dnum * 6) + a], sizeof(struct dopusfunction));
					...
					else
					{
						...
						else
						{
							...
							if(!editon)
							{
								...
								editdrivegadgets[1].Flags &= ~GFLG_SELECTED;
								AddGadgets(Window, editdrivegadgets, drivegadgets2, 4, screen_pens[config->gadgettopcol].pen, screen_pens[config->gadgetbotcol].pen, 1);
				...
				case DRIVE_EDITREQ:
					...
					{
						LStrnCpy(edit_pathbuf, buf, FILEBUF_SIZE);
...
void showdrivename(struct dopusfunction *drive, struct Gadget *gadget)
{
	...
	y = gadget->TopEdge;
	if(drive)
		SetAPen(rp, screen_pens[drive->bpen].pen);
	else
...
void driveeditoff()
{
	...
	seligad = NULL;
	SetAPen(rp, screen_pens[0].pen);
	RectFill(rp, x_off + 2, y_off + 65, x_bot, y_off + 140);
